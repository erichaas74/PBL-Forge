...
class RendererStub {
    ...
    mount(): void { /* WebGL is intentionally absent from this unit test. */ }
    dispose(): void { /* No renderer resources are allocated by the stub. */ }
    render(): void { /* Rendering is outside this unit test's scope. */ }
    syncAssembly(): void { /* The page's draft mutations are asserted directly. */ }
...
describe('SnapWorkshopPage', () => {
    ...
    const definition = ASSEMBLY_PART_DEFINITIONS.find(item => item.family === 'dragon' && item.snapPoints.length > 1);
    ...
    beforeEach(async () => {
        ...
        renderer = fixture.debugElement.injector.get(AssemblyRendererService) as unknown as RendererStub;
        draft = TestBed.inject(DesignerDragonDraftStore);
        fixture.detectChanges();

        vi.spyOn(stage(), 'setPointerCapture').mockReturnValue(undefined);
        vi.spyOn(stage(), 'releasePointerCapture').mockReturnValue(undefined);
    });
    ...
    it('offers only parts that author their own sockets', () => {
        ...
        expect(page.definitions().every(item => item.snapPoints.length > 0)).toBe(true);
        expect(page.definitions().some(item => item.family !== 'dragon')).toBe(false);
    });
    ...
    it('claims the gesture from the orbit controls for the length of the drag', () => {
        ...
        page.onPointerDown(pointer());
        expect(renderer.controlsEnabled).toBe(false);

        page.onPointerUp(pointer());
        expect(renderer.controlsEnabled).toBe(true);
    });
