...
describe('Mini Dragon Parts Lab catalog', () => {
    it('gives every shared Mini Dragon mesh profile an isolated renderable part', () => {
        const profileIds = MINI_DRAGON_PART_DEFINITIONS.map(definition => definition.visualProfile?.profileId);
        ...
        for (const definition of MINI_DRAGON_PART_DEFINITIONS) {
            ...
            expect(createMiniDragonProceduralObject(part), definition.visualProfile?.profileId ?? definition.id).toBeTruthy();
        }
    });
});
