...
describe('AssemblyGarageStore', () => {
    ...
    it('removes joints connected to a deleted part', () => {
        ...
        expect(store.state().parts.some(part => part.id === selectedPartId)).toBe(false);
        expect(store.state().joints.some(joint => joint.parentPartId === selectedPartId || joint.childPartId === selectedPartId)).toBe(false);
    });
    ...
    it('imports valid assembly JSON and rejects invalid joint references', () => {
        ...
        expect(store.partCount()).toBe(2);
        expect(store.state().isSimulating).toBe(false);
