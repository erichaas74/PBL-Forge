...
export class AssemblyArenaRendererService {
  ...
  private addArena(group: THREE.Group, setup: ArenaSetupConfig): void {
    ...
    if (pit) {
      // Layered from the ground outward so the arena reads as a built Viking
      // training complex instead of a generic physics box with decorations.
      this.addRoundFightingFloor(group, pitRadius);
      this.addStonePitRing(group, pitRadius);
      this.addRunicCenterMark(group, pitRadius);
      this.addPalisade(group, pitRadius, setup.wallHeight);
      this.addKillRingGallery(group, pitRadius, setup.wallHeight);
      this.addTieredGrandstands(group, pitRadius, setup.wallHeight);
      this.addArenaGates(group, pitRadius, setup.wallHeight);
      this.addWatchTowers(group, pitRadius, setup.wallHeight);
      this.addPalisadeShields(group, pitRadius, setup.wallHeight);
      this.addHangingBanners(group, pitRadius, setup.wallHeight);
      this.addTrainingTotems(group, pitRadius, setup.wallHeight);
  ...
  /**
   * Circular sand floor laid over the rectangular physics floor.
   *
   * Physics still owns the rectangle; this is purely a visual cap. The dark
   * scuffed ring gives the dragons an obvious combat boundary without adding
   * anything they can collide with.
   */
  private addRoundFightingFloor(group: THREE.Group, pitRadius: number): void {
    const sand = new THREE.Mesh(
      new THREE.CircleGeometry(pitRadius * 0.985, 64),
      new THREE.MeshStandardMaterial({
        color: 0xc7ad79,
        roughness: 1,
        metalness: 0,
    ...
    const wornEdge = new THREE.Mesh(
      new THREE.RingGeometry(pitRadius * 0.78, pitRadius * 0.965, 64),
      new THREE.MeshStandardMaterial({
        color: 0x8d7550,
        roughness: 1,
        metalness: 0,
    ...
    wornEdge.rotation.x = -Math.PI / 2;
    wornEdge.position.y = 0.018;
    group.add(wornEdge);
  }

  /**
   * Low stone curb around the sand. It sits just outside the playable circle,
   * so the physics wall remains authoritative while the eye reads a real pit.
   */
  private addStonePitRing(group: THREE.Group, pitRadius: number): void {
    ...
    const outer = new THREE.Mesh(
      new THREE.TorusGeometry(pitRadius + 0.08, 0.24, 7, 72),
      stone,
    );
    ...
    const inner = new THREE.Mesh(
      new THREE.TorusGeometry(pitRadius - 0.18, 0.08, 6, 72),
      new THREE.MeshStandardMaterial({
        color: 0x3e3932,
        roughness: 1,
        metalness: 0,
  ...
  /**
   * Simple carved-looking arena mark. Geometry rather than a texture means it
   * stays sharp at every quality setting and costs almost nothing.
   */
  private addRunicCenterMark(group: THREE.Group, pitRadius: number): void {
    ...
    for (const scale of [0.11, 0.22]) {
      const ring = new THREE.Mesh(
        new THREE.TorusGeometry(pitRadius * scale, 0.035, 5, 36),
        material,
      );
    ...
    const barGeometry = new THREE.BoxGeometry(pitRadius * 0.34, 0.018, 0.055);
  ...
  /**
   * Three bands of rough spectator seating outside the fighting ring.
   *
   * The front gallery remains intentionally thin for camera visibility. These
   * benches live farther back and lower, so they add the Viking-arena silhouette
   * without becoming a wall around the combatants.
   */
  private addTieredGrandstands(group: THREE.Group, pitRadius: number, wallHeight: number): void {
    const timber = new THREE.MeshStandardMaterial({
      color: 0x60472f,
      roughness: 0.96,
      metalness: 0,
    ...
    const arcs = [
      { start: Math.PI * 0.12, length: Math.PI * 0.58 },
      { start: Math.PI * 0.82, length: Math.PI * 0.36 },
      { start: Math.PI * 1.52, length: Math.PI * 0.36 },
    ];

    for (const arc of arcs) {
      for (let tier = 0; tier < 3; tier += 1) {
        const inner = pitRadius + 2.25 + tier * 0.72;
        const outer = inner + 0.52;
        const bench = new THREE.Mesh(
          new THREE.RingGeometry(inner, outer, 36, 1, arc.start, arc.length),
          tier % 2 === 0 ? timber : dark,
        );
        bench.rotation.x = -Math.PI / 2;
        bench.position.y = wallHeight * 1.05 + tier * 0.38;
        bench.receiveShadow = true;
        group.add(bench);

        const riser = new THREE.Mesh(
          new THREE.CylinderGeometry(outer, outer, 0.34, 36, 1, true, arc.start, arc.length),
          dark,
        );
        riser.position.y = bench.position.y - 0.18;
        group.add(riser);
      }
    }
  }
  ...
  /**
   * Heavy timber gates at the north and south ends of the arena.
   */
  private addArenaGates(group: THREE.Group, pitRadius: number, wallHeight: number): void {
    const timber = new THREE.MeshStandardMaterial({
      color: 0x50351f,
      roughness: 0.94,
      metalness: 0,
    ...
    const gateWidth = 2.8;
    const gateHeight = wallHeight * 1.62;

    for (const side of [-1, 1]) {
      const gate = new THREE.Group();
      gate.position.set(0, 0, side * (pitRadius + 0.5));
      gate.rotation.y = side > 0 ? Math.PI : 0;

      for (const x of [-gateWidth / 2, gateWidth / 2]) {
        const upright = new THREE.Mesh(
          new THREE.BoxGeometry(0.34, gateHeight, 0.42),
          timber,
        );
      ...
      const lintel = new THREE.Mesh(
        new THREE.BoxGeometry(gateWidth + 0.9, 0.42, 0.52),
        timber,
      );
      ...
      const hornLeft = new THREE.Mesh(
        new THREE.ConeGeometry(0.16, 0.82, 7),
        new THREE.MeshStandardMaterial({ color: 0xc3aa78, roughness: 0.85 }),
      );
      hornLeft.position.set(-gateWidth * 0.34, gateHeight + 0.5, 0);
      hornLeft.rotation.z = -0.34;
      gate.add(hornLeft);

      const hornRight = hornLeft.clone();
      hornRight.position.x = gateWidth * 0.34;
      hornRight.rotation.z = 0.34;
      gate.add(hornRight);

      for (let index = -2; index <= 2; index += 1) {
        const bar = new THREE.Mesh(
          new THREE.CylinderGeometry(0.055, 0.055, gateHeight * 0.86, 6),
          iron,
        );
        bar.position.set(index * 0.46, gateHeight * 0.43, 0.08);
        gate.add(bar);
      }
  ...
  /**
   * Four crude lookout towers beyond the palisade.
   */
  private addWatchTowers(group: THREE.Group, pitRadius: number, wallHeight: number): void {
    const timber = new THREE.MeshStandardMaterial({
      color: 0x4d3523,
      roughness: 0.97,
      metalness: 0,
    ...
    const towerHeight = wallHeight * 2.45;
    const distance = pitRadius + 2.55;
    ...
    for (const angle of [Math.PI / 4, Math.PI * 3 / 4, Math.PI * 5 / 4, Math.PI * 7 / 4]) {
      ...
      for (const x of [-0.55, 0.55]) {
        for (const z of [-0.42, 0.42]) {
          const leg = new THREE.Mesh(
            new THREE.CylinderGeometry(0.11, 0.15, towerHeight, 6),
            timber,
          );
      ...
      const platform = new THREE.Mesh(
        new THREE.BoxGeometry(1.7, 0.22, 1.35),
        timber,
      );
      platform.position.y = towerHeight * 0.78;
      ...
      roof.position.y = towerHeight * 0.78 + 0.72;
  ...
  /**
   * Cloth banners hanging from the gallery rail. No texture dependency: the
   * triangular tails and muted dyes are enough to read at arena distance.
   */
  private addHangingBanners(group: THREE.Group, pitRadius: number, wallHeight: number): void {
    const colours = [0x7a3027, 0x3f5663, 0xa07a32, 0x6a6847];
    const deckY = wallHeight * 1.7;
    const radius = pitRadius + 0.48;

    for (let index = 0; index < 12; index += 1) {
      const angle = (index / 12) * Math.PI * 2 + 0.15;
      const banner = new THREE.Mesh(
        new THREE.PlaneGeometry(0.48, 1.05),
        new THREE.MeshStandardMaterial({
      ...
      banner.rotation.z = Math.sin(index * 1.77) * 0.05;
      group.add(banner);
    }
  }

  /**
   * Carved dragon-training posts just outside the combat ring.
   *
   * They are scenery, not obstacles, and deliberately generic: horned wooden
   * targets suggest a dragon-training culture without copying a specific creature.
   */
  private addTrainingTotems(group: THREE.Group, pitRadius: number, wallHeight: number): void {
    ...
    const radius = pitRadius + 1.25;
    for (const angle of [0, Math.PI / 2, Math.PI, Math.PI * 1.5]) {
      const totem = new THREE.Group();
      totem.position.set(Math.cos(angle) * radius, 0, Math.sin(angle) * radius);
      totem.rotation.y = -angle;

      const post = new THREE.Mesh(
        new THREE.CylinderGeometry(0.12, 0.17, wallHeight * 1.15, 7),
        wood,
      );
      post.position.y = wallHeight * 0.575;
      post.castShadow = true;
      totem.add(post);

      const head = new THREE.Mesh(
        new THREE.BoxGeometry(0.66, 0.42, 0.3),
        wood,
      );
      head.position.y = wallHeight * 1.13;
      ...
      for (const side of [-1, 1]) {
        ...
        horn.position.set(side * 0.29, wallHeight * 1.35, 0);
  ...
  private addBraziers(group: THREE.Group, pitRadius: number, wallHeight: number): void {
    ...
    for (let index = 0; index < 6; index += 1) {
      ...
      const light = new THREE.PointLight(BERK_MATERIALS.emberGlow, 16, pitRadius * 2.5, 2);
