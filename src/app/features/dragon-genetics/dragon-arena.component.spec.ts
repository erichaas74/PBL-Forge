...
import { BattleBodySnapshot, ControlFrameByCombatant, } from '../../shared/assembly-arena/models/arena.models';
...
@Component({
    ...
    template: '<div class="viewport-stub" [attr.data-appearance]="appearance()" [attr.aria-label]="ariaLabel()"></div>',
})
...
describe('DragonArenaComponent', () => {
    ...
    beforeEach(async () => {
        ...
        fixture.componentRef.setInput('champion', runDragonBatch(findParent('ember'), findParent('tide'), 7, 1).sample[0]);
    ...
    it('renders the Viking arena shell around the shared viewport', () => {
        ...
        expect(element.querySelector('.field-guide')?.hasAttribute('open')).toBe(false);
    });
    ...
    it('changes command mode while keeping the same combatants', () => {
        ...
        const tacticalButton = [...element.querySelectorAll<HTMLButtonElement>('.mode-button')].find((button) => button.textContent?.includes('Tactical turns'));

        tacticalButton?.click();
        fixture.detectChanges();

        expect(fixture.componentInstance.arena.state().matchId).toBeGreaterThan(matchId);
        expect(fixture.componentInstance.arena.state().combatants.map((combatant) => combatant.name)).toEqual(combatants);
    ...
    it('shows inherited move availability in the control deck', () => {
        const buttons = [
            ...(fixture.nativeElement as HTMLElement).querySelectorAll<HTMLButtonElement>('.attack-controls button'),
        ];
        ...
        expect(wing?.disabled).toBe(false);
        expect(horn?.disabled).toBe(true);
