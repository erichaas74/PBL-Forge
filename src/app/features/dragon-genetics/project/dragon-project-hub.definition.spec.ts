...
describe('Dragon project hub definition', () => {
    ...
    it('starts with one shared-foundation action', () => {
        const view = buildProjectHubViewModel(DRAGON_PROJECT_HUB_DEFINITION, assignment, studentState());

        expect(view.requiredActivityCount).toBe(6);
        expect(view.nextAction).toEqual(expect.objectContaining({ kind: 'activity', activityId: 'trait-evidence' }));
    ...
    it('offers one final-path choice after shared work is complete', () => {
        const progress = Object.fromEntries(DRAGON_PROJECT_HUB_DEFINITION.activities
            .filter((activity) => activity.required)
            .filter((activity) => !DRAGON_PROJECT_HUB_DEFINITION.paths?.some((path) => path.activityIds.includes(activity.id)))
            .map((activity) => [
            activity.id,
            {
        ...
        const view = buildProjectHubViewModel(DRAGON_PROJECT_HUB_DEFINITION, assignment, studentState(progress));

        expect(view.progressPercent).toBe(100);
        expect(view.nextAction).toEqual(expect.objectContaining({ kind: 'choose-path' }));
    });
});

function studentState(activityProgress: StudentProjectState['activityProgress'] = {}): StudentProjectState {
    return {
        studentId: 'student-1',
