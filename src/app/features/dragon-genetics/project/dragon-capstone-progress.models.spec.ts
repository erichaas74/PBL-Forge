...
import { summarizeDragonArena, summarizeIslandDiversity, summarizeMiniDragonShow, } from './dragon-capstone-progress.models';
...
describe('Dragon capstone progress summaries', () => {
    ...
    it('uses the registered show entry as the completed outcome', () => {
            ...
            registry: [
                {
                    ...
                    inbreedingPercent: 0,
                    ribbons: 4,
                    showDivisionId: 'hearth-companion' as const,
                    showRunId: 'show-1',
                    geneticScore: 43,
                    trainingScore: 37.5,
                    combinedScore: 80.5,
        ...
        expect(summarizeMiniDragonShow(snapshot)).toEqual(expect.objectContaining({
            status: 'complete',
            breedName: 'Cloud Puff',
            consistencyPercent: 84,
            ribbons: 4,
    ...
    it('completes Island Diversity after every population has been managed', () => {
        ...
        for (const islandId of ISLAND_IDS)
            world = advanceIslandGeneration(world, islandId);
