...
import {
  ...
  hatcheryClutchCopy,
} from '../simulation/data/dragon-hatchery-content';
import {
  DRAGON_TRAITS,
  genotypeLabel,
  getTrait,
...
import {
  ...
  HatcheryRunRecord,
} from '../simulation/domain/dragon-hatchery.models';
import {
  DragonOffspring,
  DragonParentProfile,
  DragonTraitId,
} from '../simulation/domain/dragon-lab.models';
import { createDragonHatcheryScene } from '../visual-adapter/dragon-visual-scene.adapter';
...
@Component({
  ...
  imports: [DragonHatcheryDisplayComponent],
...
export class DragonHatcheryStationComponent {
  ...
  private readonly run = linkedSignal<RunState>(() => createRun({
    clutch: this.clutch(),
    mode: this.mode(),
  ...
  readonly actualDominantCount = computed(() => {
    ...
    return this.clutch().filter(egg => showsDominantPhenotype(egg.genome[trait.id], trait.id)).length;
  });
  readonly hatchedDominantCount = computed(() => {
    const trait = this.focusTrait();
    if (!trait) return null;
    return this.hatchedEggs()
      .filter(egg => showsDominantPhenotype(egg.genome[trait.id], trait.id)).length;
  });

  readonly copy = computed(() => hatcheryClutchCopy({
    clutchId: this.clutchId(),
    ...
    switch (run.phase) {
      ...
      case 'reveal':
        return run.hatchCommitted
          ? {
            label: this.evidence().length ? 'Explain what the records show' : 'Save the hatchery record',
            disabled: false,
  ...
  runPrimaryAction(): void {
    ...
    switch (action.kind) {
      ...
      case 'stage':
        ...
        this.feedbackState.set(this.tools().includes('hatch')
          ? {
            tone: 'neutral',
  ...
  eggPhenotype(eggId: string): string {
    ...
    const trait = this.focusTrait();
    return trait
      ? phenotypeLabel(egg, trait.id)
      : this.clutchTraitSummary(egg);
  }
  ...
  private saveRun(): void {
    ...
    const record: HatcheryRunRecord = {
      ...
      predictionCorrect: run.prediction === null || actual === null
        ? null
        : run.prediction === actual,
      evidenceMarkId: run.evidenceMarkId,
      evidenceCorrect,
