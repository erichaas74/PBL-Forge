...
export class GenomeMicroscopeStationComponent {
  ...
  readonly primaryAction = computed<PrimaryAction>(() => {
    ...
    if (run.phase === 'observe') return { label: 'Begin prediction', disabled: false, kind: 'begin' };
    if (run.phase === 'predict') return { label: 'Lock a microscope level above', disabled: true, kind: 'waiting' };
    ...
    if (run.phase === 'reveal') return { label: 'Explain with evidence', disabled: false, kind: 'explain' };
    return {
  ...
  readonly stepHint = computed(() => {
    switch (this.run().phase) {
      case 'observe': return 'Observe: inspect the generic sample record and read the microscope assignment.';
      case 'predict': return 'Predict: choose the level before any allele symbols are revealed.';
      case 'manipulate': return 'Map: select a hierarchy label, then select its numbered bay. Dragging also works.';
      case 'reveal': return 'Reveal: follow nucleus → chromosome → DNA → gene → allele pair.';
      case 'explain': return 'Explain: pin the level that directly supports your answer.';
      default: return 'All microscope evidence has been saved.';
    }
  });
    ...
    switch (event.type) {
      ...
      case 'label-placed':
        if (isGenomeLevel(event.targetId) && isGenomeLevel(event.value)) {
          this.placeLabel(event.targetId as DragonGenomeLevelId, event.value as DragonGenomeLevelId);
        }
        break;
  ...
  private placeLabel(labelId: DragonGenomeLevelId, levelId: DragonGenomeLevelId): void {
    ...
    if (run.placements.some(placement =>
      placement.labelId === labelId || placement.levelId === levelId)) return;
  ...
  private checkMap(): void {
    ...
    this.feedbackState.set(this.mode() === 'official'
      ? {
        ...
        detail: 'Follow the containment path and pin the level that supports your submitted prediction.',
      }
      : {
        tone: predictionCorrect ? 'good' : 'warn',
        headline: predictionCorrect ? 'Prediction supported.' : 'The sample resolved at a different level.',
  ...
  private pinEvidence(level: DragonGenomeLevelId): void {
    ...
    this.feedbackState.set(this.mode() === 'official'
      ? { tone: 'neutral', headline: 'Evidence recorded.', detail: 'Save this microscope record when ready.' }
      : {
        tone: correct ? 'good' : 'warn',
        headline: correct ? 'This level directly supports the claim.' : 'That level is part of the path, but it is not the strongest evidence.',
        detail: correct ? this.activeTask()?.explanation : 'Choose the level named by the assignment and compare its definition.',
      });
  }
    ...
    if (run.activeIndex >= run.taskIds.length - 1) {
      ...
      this.setCompleted.emit({
        ...
        correct: records.filter(item =>
          item.predictionCorrect && item.hierarchyCorrect && item.evidenceCorrect).length,
