...
function stageEvent(
  ...
  return { sceneId: 'module-6-dragon-hatchery-learn', type, targetId, value, occurredAtIso: '2026-01-01T00:00:00.000Z' };
}
...
describe('DragonHatcheryStationComponent', () => {
  ...
  function instrument(): DragonHatcheryInstrument {
    ...
    if (scene?.instrument.kind !== 'dragon-hatchery') throw new Error('No hatchery scene published.');
  ...
  it('publishes a valid clutch scene with every egg sealed', () => {
    ...
    expect(instrument().eggs.every(egg => !egg.examined && !egg.sampled && !egg.hatched)).toBeTrue();
  ...
  it('records candling and sampling separately for each egg', () => {
    ...
    expect(instrument().eggs.slice(1).every(egg => !egg.examined && !egg.sampled)).toBeTrue();
  });
