...
describe('Dragon Arena evidence', () => {
  ...
  it('describes modeled combat effects and keeps scale pattern neutral', () => {
    ...
    expect(evidence.find((record) => record.traitId === 'wings')).toEqual(
      jasmine.objectContaining({ genotype: 'Ww', kind: 'ability' }),
    );
    expect(evidence.find((record) => record.traitId === 'fire')?.arenaEffect).toBe(
      'Fire breath unavailable.',
    );
    expect(evidence.find((record) => record.traitId === 'scales')).toEqual(
