...
describe('Island Diversity population domain', () => {
  ...
  it('lets a protected pair contribute offspring while wild matings continue', () => {
    ...
    const adults = world.islands.sanctuary.dragons.filter(
      (dragon) =>
        isBreedingAdult(dragon) && dragon.ageGenerations <= 3 && !isMoonfadeAffected(dragon),
    );
    ...
    const offspring = next.islands.sanctuary.dragons.filter(
      (dragon) => dragon.ageGenerations === 0,
    );
    ...
    expect(offspring.slice(0, 2).every((dragon) => dragon.parents.includes(female.id))).toBeTrue();
    expect(offspring.slice(0, 2).every((dragon) => dragon.parents.includes(male.id))).toBeTrue();
    expect(offspring.some((dragon) => !dragon.parents.includes(female.id))).toBeTrue();
  });
  ...
  it('reconstructs generation outcomes deterministically from a student seed', () => {
    ...
    expect(first.islands['ash-island'].timeline.at(-1)?.event).toBe(
      second.islands['ash-island'].timeline.at(-1)?.event,
    );
  });
  ...
  it('distinguishes healthy carriers from affected recessive dragons', () => {
    ...
    expect(dragons.some(isMoonfadeCarrier)).toBeTrue();
    expect(dragons.some(isMoonfadeAffected)).toBeTrue();
    expect(
      dragons.every((dragon) => !(isMoonfadeCarrier(dragon) && isMoonfadeAffected(dragon))),
    ).toBeTrue();
  });
});
