...
describe('IslandDiversityManagerComponent', () => {
    ...
    it('supports both click and drag paths for individual relocation', () => {
        ...
        expect(component.world().islands['founders-isle'].dragons.some((dragon) => dragon.id === first.id)).toBe(true);
        ...
        expect(component.world().islands.moonmist.dragons.some((dragon) => dragon.id === second.id)).toBe(true);
    });
    ...
    it('supports click and drag paths for protected breeding berths', () => {
        ...
        const adults = component
            .selectedPopulation()
            .dragons.filter((dragon) => isBreedingAdult(dragon) &&
    ...
    it('admits an account dragon through click and chromosome-drop paths without revealing its genotype', () => {
        ...
        component.dropOnIsland(dragEvent(ACCOUNT_GENETICS_RECORD_DRAG_TYPE, JSON.stringify({ kind: 'chromosome', id: chromosome.id })), 'sanctuary');
    ...
    it('saves a student-built island note in the persistent world', () => {
        component.updateNote('Stormbreak has few lineages, so I am comparing diversity before and after one relocation.');
        component.saveFieldNote();

        expect(component.world().notes.stormbreak?.text).toContain('few lineages');
        expect(component.ledgerOpen()).toBe(false);
    });
});
