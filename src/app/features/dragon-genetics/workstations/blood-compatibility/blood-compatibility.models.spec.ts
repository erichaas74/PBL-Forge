...
describe('dragon blood compatibility model', () => {
  ...
  it('expresses A and B codominantly and O without an A or B antigen', () => {
    ...
    expect(antiserumReaction(dual, 'a')).toBeTrue();
    expect(antiserumReaction(dual, 'b')).toBeTrue();
    expect(antiserumReaction(aCarrier, 'a')).toBeTrue();
    expect(antiserumReaction(aCarrier, 'b')).toBeFalse();
  ...
  it('accepts donor cells only when every donor marker is present on the recipient', () => {
    ...
    for (const recipient of [oType, aType, bType, abType]) {
      expect(transfusionCompatibility(oType, recipient).compatible).toBeTrue();
    }
    expect(transfusionCompatibility(aType, aType).compatible).toBeTrue();
    expect(transfusionCompatibility(aType, abType).compatible).toBeTrue();
    expect(transfusionCompatibility(aType, bType)).toEqual({
      compatible: false,
      unfamiliarMarkers: ['a'],
    });
    expect(transfusionCompatibility(bType, bType).compatible).toBeTrue();
    expect(transfusionCompatibility(bType, abType).compatible).toBeTrue();
    expect(transfusionCompatibility(bType, aType)).toEqual({
      compatible: false,
      unfamiliarMarkers: ['b'],
    });
    expect(transfusionCompatibility(abType, abType).compatible).toBeTrue();
    expect(transfusionCompatibility(abType, aType).compatible).toBeFalse();
    expect(transfusionCompatibility(abType, bType).compatible).toBeFalse();
    expect(transfusionCompatibility(abType, oType).compatible).toBeFalse();
  });
  ...
  it('provides one neutral-coded clinic donor for every marker phenotype', () => {
    ...
    expect(new Set(phenotypes)).toEqual(
      new Set(['o-positive', 'a-positive', 'b-positive', 'ab-positive']),
    );
