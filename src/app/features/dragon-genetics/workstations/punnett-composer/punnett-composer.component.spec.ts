...
describe('PunnettComposerComponent', () => {
    ...
    it('starts with separate female and male dragon chromosome selectors', () => {
        ...
        expect(selectors[0].dragons().every((dragon) => dragon.sex === 'female')).toBe(true);
        expect(selectors[1].dragons().every((dragon) => dragon.sex === 'male')).toBe(true);
        expect((fixture.nativeElement as HTMLElement).querySelector('app-account-genetics-file')).toBeNull();
        expect(selectors.every((selector) => selector.cellChromosomes().length === 5)).toBe(true);
    ...
    it('replaces the fixed locus choices with genes from the selected chromosome', () => {
        const selector = fixture.debugElement.queryAll(By.directive(DragonChromosomeSelectorComponent))[0].componentInstance as DragonChromosomeSelectorComponent;
    ...
    it('loads each parents two chromosome copies when its card gene is selected', () => {
        ...
        expect(composer.squareComplete()).toBe(true);
    ...
    it('uses standardized heterozygous XX and XY cells in test mode', () => {
        ...
        fixture.detectChanges();
        expect(fixture.debugElement.queryAll(By.directive(CellChromosomeViewportComponent)).length).toBe(2);
        expect(fixture.nativeElement.querySelectorAll('.test-model .gamete-token').length).toBe(4);
        expect(fixture.nativeElement.querySelector('.test-model app-dragon-chromosome-selector')).toBeNull();
    ...
    it('drags a test-model gene copy into its matching Punnett axis box', () => {
        ...
        composer.dropGamete({ dataTransfer, preventDefault: () => undefined } as unknown as DragEvent, 'parent1', 0);
...
function loadWingCross(composer: PunnettComposerComponent, library: AccountGeneticsLibraryService, studentId: string): void {
    const dragons = library.recordsFor(studentId).dragons;
    composer.loadAccountRecord(dragons.find((dragon) => dragon.id === 'ember')!, 'parent1');
    composer.loadAccountRecord(dragons.find((dragon) => dragon.id === 'tide')!, 'parent2');
