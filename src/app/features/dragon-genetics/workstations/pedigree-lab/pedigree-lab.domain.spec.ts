...
import { assessRelatedness, bloodlineStats, breedClutch, descendantIds, kinshipCoefficient, lineToAncestor, mergeHatchlings, } from './pedigree-lab.domain';
import { ARCHIVE_YEAR, PEDIGREE_GENE_IDS } from './pedigree-lab.models';
import { PEDIGREE_ARCHIVE, archiveDragon, investigationById, transmissibleAlleles, } from './pedigree-population';
...
describe('bloodline statistics', () => {
    ...
    it('counts carriers only once a model has been chosen', () => {
        ...
        const withModel = bloodlineStats(PEDIGREE_ARCHIVE, FROST, deducePedigree({
...
describe('pedigree walks', () => {
    ...
    it('counts descendants without counting the ancestor', () => {
        ...
        expect(descendants.has('vyrak')).toBe(false);
        expect(descendants.has('kaenor')).toBe(true);
        expect(descendants.has('arkon')).toBe(true);
        expect(descendants.has('korrak')).toBe(false);
    });
});
...
describe('breeding a clutch', () => {
    ...
    it('gives every hatchling one allele from each parent', () => {
        for (const hatchling of hatch(1).hatchlings) {
            for (const geneId of PEDIGREE_GENE_IDS) {
                ...
                expect(maternal.some((allele) => pair.includes(allele)), `${hatchling.id} at ${geneId}`).toBe(true);
                expect(paternal.some((allele) => pair.includes(allele)), `${hatchling.id} at ${geneId}`).toBe(true);
            }
        }
    });
    ...
    it('samples each egg independently rather than dealing out the expected ratio', () => {
        ...
        expect(first.hatchlings.map((h) => h.genome.scales.join(''))).toEqual(hatch(1).hatchlings.map((h) => h.genome.scales.join('')));
    ...
    it('records the clutch against the prediction it was authorised on', () => {
        ...
        const recovered = outcome.hatchlings.filter((hatchling) => truePhenotype(FROST.geneId, hatchling.genome[FROST.geneId]) === FROST.lostPhenotype);

        expect(outcome.record.predictedPercent).toBe(25);
        expect(outcome.record.recoveredCount).toBe(recovered.length);
        expect(outcome.record.inbreedingCoefficient).toBeCloseTo(kinshipCoefficient(PEDIGREE_ARCHIVE, mother.id, father.id), 6);
    });
    ...
    it('links hatchlings into the pedigree so they descend from the legend', () => {
        ...
        expect(descendantIds(merged, 'vyrak').has(hatchling.id)).toBe(true);
