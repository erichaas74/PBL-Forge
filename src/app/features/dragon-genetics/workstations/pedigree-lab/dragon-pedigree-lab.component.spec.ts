...
describe('DragonPedigreeLabComponent', () => {
    ...
    it('redraws the canvas around the dragon that was selected', () => {
        ...
        expect(drawn.has('ivrid')).toBe(true);
        expect(drawn.has('kaenor'), 'father').toBe(true);
        expect(drawn.has('orsa'), 'mother').toBe(true);
        expect(drawn.has('hesper'), 'daughter').toBe(true);
        expect(drawn.has('halgrim'), 'brother').toBe(false);
        expect(drawn.has('aster'), 'sister').toBe(false);
        expect(drawn.has('sindri'), 'cousin').toBe(false);
    ...
    it('brings the whole bloodline back when the ancestor is reopened', () => {
        ...
        fixture.detectChanges();
        expect(lab.canvasNodes().some((node) => node.dragonId === 'sindri')).toBe(false);

        lab.selectDragon(lab.investigation().ancestorId);
        fixture.detectChanges();

        const drawn = new Set(lab.canvasNodes().map((node) => node.dragonId));
        expect(drawn.has('vyrak')).toBe(true);
        expect(drawn.has('sindri')).toBe(true);
        expect(drawn.has('ivrid')).toBe(true);
    });
    ...
    it('spends a limited sequencing budget and refuses damaged samples', () => {
        ...
        expect(lab.deduction()?.states.get('arkon')?.sequenced).toBe(true);
    ...
    it('requires evidence, a prediction, and a reason before authorising a breeding', () => {
        ...
        lab.addToTray('arkon');
        lab.addToTray('sylva');
        expect(lab.canAuthorize()).toBe(false);
        ...
        expect(lab.canAuthorize()).toBe(true);
