...
import { PEDIGREE_GENE_IDS, PedigreeDragon, pedigreeGene, tracedAllele, } from './pedigree-lab.models';
import { BLOODLINE_INVESTIGATIONS, PEDIGREE_ARCHIVE, PEDIGREE_UNIONS, archiveDragon, normalizePair, parseGenotypeCode, transmissibleAlleles, } from './pedigree-population';
...
describe('pedigree archive', () => {
    ...
    it('draws every gene from the shared expressive trait catalog', () => {
        for (const geneId of PEDIGREE_GENE_IDS) {
            expect(EXPRESSIVE_DRAGON_TRAITS.some((trait) => trait.id === geneId)).toBe(true);
            expect(pedigreeGene(geneId)).toBe(EXPRESSIVE_DRAGON_TRAITS.find((trait) => trait.id === geneId)!);
        }
    });

    it('gives every dragon a genotype its recorded parents could actually transmit', () => {
        for (const dragon of PEDIGREE_ARCHIVE) {
            if (!dragon.motherId || !dragon.fatherId)
                continue;
            const mother = byId.get(dragon.motherId);
            const father = byId.get(dragon.fatherId);
            expect(mother, `mother of ${dragon.id}`).toBeDefined();
            expect(father, `father of ${dragon.id}`).toBeDefined();
            if (!mother || !father)
                continue;
            ...
            for (const geneId of PEDIGREE_GENE_IDS) {
                ...
                const reachable = maternal.flatMap((fromMother) => paternal.map((fromFather) => normalizePair(geneId, [fromMother, fromFather]).join('')));
                expect(reachable, `${dragon.id} at ${geneId}`).toContain(dragon.genome[geneId].join(''));
            }
        }
    });
    ...
    it('honours every genotype the written history pins', () => {
        for (const union of PEDIGREE_UNIONS) {
            for (const child of union.children) {
                ...
                expect(dragon, child.id).toBeDefined();
                if (!dragon)
                    continue;
                for (const [geneId, code] of Object.entries(child.require)) {
                    const expected = parseGenotypeCode(geneId as (typeof PEDIGREE_GENE_IDS)[number], code as string, dragon.sex);
                    expect(dragon.genome[geneId as (typeof PEDIGREE_GENE_IDS)[number]].join(''), `${child.id} pinned at ${geneId}`).toBe(expected.join(''));
                }
            }
        }
    });
    ...
    it('leaves every hunted appearance absent from the living register', () => {
        for (const investigation of BLOODLINE_INVESTIGATIONS) {
            const livingWithTrait = PEDIGREE_ARCHIVE.filter((dragon) => dragon.alive &&
            ...
            expect(observedPhenotypeOf(legend, investigation.geneId), investigation.id).toBe(investigation.lostPhenotype);
            expect(livingWithTrait.map((dragon) => dragon.id), `${investigation.id} should be lost among the living`).toEqual([]);
        }
    });
    ...
    it('keeps the frost line traceable: Vyrak passes his allele to every child', () => {
        ...
        for (const childId of vyrak.offspringIds) {
            ...
            expect(observedPhenotypeOf(child, 'scales')).not.toBe(BLOODLINE_INVESTIGATIONS[0].lostPhenotype);
        }
    });
    ...
    it('keeps one record incomplete so pedigree gaps have to be reasoned around', () => {
        ...
        expect(brandt.dnaAvailable).toBe(false);
