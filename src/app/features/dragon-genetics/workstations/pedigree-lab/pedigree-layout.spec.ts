...
describe('pedigree layout', () => {
  ...
  it('centres a sibship under the couple it descends from', () => {
    ...
    for (const union of result.unions) {
      ...
      const centre =
        union.childXs.reduce((sum, value) => sum + value, 0) / union.childXs.length;
      ...
      expect(Math.abs(centre - midpoint))
        .withContext(`${union.id} sibship centre`)
        .toBeLessThanOrEqual(PEDIGREE_NODE_SPACING / 2);
    }
  });
  ...
  it('never leaves a descent line hanging beside its sibship', () => {
    for (const focusId of ['vyrak', 'sable', 'ilira', 'korrak', 'arkon']) {
      ...
      for (const union of result.unions) {
        ...
        expect(midpoint).withContext(`${focusId}/${union.id}`).toBeGreaterThanOrEqual(start);
        expect(midpoint).withContext(`${focusId}/${union.id}`).toBeLessThanOrEqual(end);
      }
    }
  });
    ...
    for (const [generation, xs] of byRow) {
      ...
      for (let index = 1; index < sorted.length; index += 1) {
        expect(sorted[index] - sorted[index - 1])
          ...
          .toBeGreaterThanOrEqual(PEDIGREE_NODE_SPACING - 0.001);
      }
    }
  });
  ...
  it('opens only the generations the student asked for', () => {
    ...
    expect(shallow.has('vyrak')).toBeTrue();
    expect(shallow.has('kaenor')).toBeTrue();
    // A grandchild is two steps down and stays closed at this depth.
    expect(shallow.has('ivrid')).toBeFalse();
  });
});
