import { ALLELE_VAULT_ALLELES, ALLELE_VAULT_GENES, AlleleVaultAllele, AlleleVaultGene, expressedAllelePairPhenotype, normalizeAlleleVaultGeneIds, } from './allele-vault.models';
...
describe('allelePairToExpressiveProfile', () => {
    ...
    it('models the released X-linked eye locus on XX and XY specimens', () => {
        ...
        const female = allelePairToExpressiveProfile(gene, [findAllele('eye-color-E'), findAllele('eye-color-e')], 'female');
        const male = allelePairToExpressiveProfile(gene, [findAllele('eye-color-e'), findAllele('eye-color-e')], 'male');
    ...
    it('adds the X-linked locus when restoring the former complete default catalog', () => {
        const formerDefault = ALLELE_VAULT_GENES.filter((gene) => gene.chromosome !== 'Chr X').map((gene) => gene.id);

        expect(normalizeAlleleVaultGeneIds(formerDefault)).toEqual(ALLELE_VAULT_GENES.map((gene) => gene.id));
    ...
    it('changes only the active locus on the controlled reference dragon', () => {
        ...
        const profile = allelePairToExpressiveProfile(gene, [findAllele('wings-w'), findAllele('wings-w')], 'female');
    ...
    it('supports male anatomy while normalizing the X-linked background', () => {
        const profile = allelePairToExpressiveProfile(findGene('horns'), [findAllele('horns-H'), findAllele('horns-h')], 'male');
    ...
    it('returns no specimen until both alleles belong to the active gene', () => {
        ...
        expect(allelePairToExpressiveProfile(gene, [findAllele('fire-F'), findAllele('horns-h')], 'female')).toBeNull();
    });
