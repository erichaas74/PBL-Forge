...
describe('AlleleVaultWorkbenchComponent', () => {
  it('keeps the 3D workstation behind an OnPush change-detection boundary', () => {
    ...
    it('renders the complete vault and a split genome comparison', () => {
        ...
        expect(body?.firstElementChild?.classList.contains('gene-selector')).toBe(true);
        ...
        expect(element.querySelectorAll('.vault-cell-viewport [data-band-start]').length).toBeGreaterThan(0);
        ...
        expect([...element.querySelectorAll('.allele-token-chromosome [data-barcode-variant]')].map((barcode) => barcode.getAttribute('data-barcode-variant'))).toEqual(['reference', 'substitution']);
    ...
    it('shows the generic XY genome as homologous pairs without sister-chromatid controls', () => {
        const element = fixture.nativeElement as HTMLElement;
        const formButtons = element.querySelectorAll<HTMLButtonElement>('.vault-cell-viewport .chromosome-form-controls button');

        expect(formButtons.length).toBe(0);
        expect(element.querySelectorAll('.vault-cell-viewport .chromosome-svg--replicated').length).toBe(6);
        ...
        expect(element
            ...
            ?.getAttribute('aria-label')).toContain('heterozygous XY');
        expect([...element.querySelectorAll('.vault-cell-viewport .chromosome-in-cell')].every((item) => item.getAttribute('aria-label')?.includes('homologous pair'))).toBe(true);
    ...
    it('keeps each allele barcode identity when it is dropped into either comparison sample', () => {
        ...
        expect(element
            ...
            ?.getAttribute('data-barcode-variant')).toBe('substitution');
        expect(element
            .querySelector('.variant-pane [data-barcode-variant]')
            ?.getAttribute('data-barcode-variant')).toBe('reference');
    });
    ...
    it('populates the gene and allele controls from the selected chromosome', () => {
        ...
        expect([...element.querySelectorAll<HTMLButtonElement>('.gene-button')].map((button) => button.textContent?.replace(/\s+/g, ' ').trim())).toEqual(['CH2-G1', 'CH2-G2', 'CH2-G3']);
        expect(element.querySelector('.vault-cell-viewport [data-chromosome="Chr 2"]')?.classList).toContain('chromosome-in-cell--selected');
        ...
        expect([...element.querySelectorAll<HTMLButtonElement>('.allele-token')].map((button) => button.getAttribute('aria-label'))).toEqual(['Allele sample CH2-G1a', 'Allele sample CH2-G1b']);
    });
    ...
    it('colors every gene control and opens the released X-linked gene', () => {
        ...
        expect([...element.querySelectorAll<HTMLElement>('.allele-token')].every((token) => token.dataset['geneColor'] === eyeGene.locusColor)).toBe(true);
    });

    it('shows only chromosomes released by the teacher list', () => {
        const releasedGeneIds = new Set(['wings', 'fire']);
        fixture.componentRef.setInput('genes', ALLELE_VAULT_GENES.filter((gene) => releasedGeneIds.has(gene.id)));
        fixture.componentRef.setInput('alleles', ALLELE_VAULT_ALLELES.filter((allele) => releasedGeneIds.has(allele.geneId)));
        fixture.detectChanges();

        const selectors = [
            ...(fixture.nativeElement as HTMLElement).querySelectorAll<HTMLButtonElement>('.vault-cell-viewport .chromosome-in-cell'),
        ];
    ...
    it('accepts question data to focus and configure a student task', () => {
        ...
        expect(component.requestedPairComplete()).toBe(false);
    ...
    it('renders and records automatically after the second allele is installed', () => {
        ...
        expect(events.some((event) => event.type === 'allele-installed')).toBe(true);
        expect(events.some((event) => event.type === 'expression-run')).toBe(true);
    });
    ...
    it('uses reference samples A and B as the genotype loading targets', () => {
        ...
        expect(component.pairIds()).toEqual(['wings-W', 'wings-w']);
        expect(component.phenotypeSource()?.kind).toBe('descriptor');
        expect((fixture.nativeElement as HTMLElement).querySelectorAll('.genome-pane.sample-loaded').length).toBe(2);
        expect((fixture.nativeElement as HTMLElement).querySelector('.chromosome-svg--placeholder')).toBeNull();
        expect((fixture.nativeElement as HTMLElement).querySelectorAll('.genome-pane .gene-locus').length).toBe(6);
    ...
    it('emits the persistent gene and allele combination for DNA analysis', () => {
        ...
        const launch = (fixture.nativeElement as HTMLElement).querySelector<HTMLButtonElement>('.dna-analysis-link');
        launch?.click();
    ...
    it('selects a gene by clicking its locus on the selector chromosome', () => {
        const locus = (fixture.nativeElement as HTMLElement).querySelector<SVGGElement>('.vault-cell-viewport .inspection-panel [data-locus="CH1-G2"]');
    ...
    it('builds a gene claim from the selected trait and allele placements', () => {
        ...
        expect(component.claimComplete()).toBe(true);
