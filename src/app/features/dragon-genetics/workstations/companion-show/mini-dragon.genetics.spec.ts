...
describe('mini dragon inheritance patterns', () => {
    ...
    it('blends feather coverage into a visible fringe', () => {
        expect(miniPhenotypeFormId('plumage', genomeWith({ plumage: ['P', 'P'] }))).toBe('plumage:full');
        expect(miniPhenotypeFormId('plumage', genomeWith({ plumage: ['P', 'p'] }))).toBe('plumage:fringe');
        expect(miniPhenotypeFormId('plumage', genomeWith({ plumage: ['p', 'p'] }))).toBe('plumage:bare');
    });
    ...
    it('shows both alleles at once for the codominant coat pattern', () => {
        ...
        expect(miniPhenotypeFormId('pattern', genomeWith({ pattern: ['G', 'A'] }))).toBe('pattern:ash-gold');
    });
        ...
        for (const gene of MINI_DRAGON_GENES) {
            for (const form of gene.forms) {
                for (const allele of gene.alleles) {
                    expect(form.label.split(/\s+/), form.id).not.toContain(allele);
                }
            }
        }
    });
...
describe('mini dragon breeding', () => {
    ...
    it('is deterministic for the same parents and seed', () => {
        ...
        expect(breedMiniGenomes(dam, sire, 'litter-1:0')).toEqual(breedMiniGenomes(dam, sire, 'litter-1:0'));
    });

    it('produces all three forms from a pair of heterozygotes at an incomplete locus', () => {
        const parent = genomeWith({ wings: ['W', 'w'] });
        const forms = new Set(Array.from({ length: 60 }, (_, index) => miniPhenotypeFormId('wings', breedMiniGenomes(parent, parent, `w:${index}`))));
...
describe('the founding population', () => {
    it('carries every allele of every gene somewhere in the pool', () => {
        for (const gene of MINI_DRAGON_GENES) {
            ...
            expect(present, gene.id).toEqual(new Set(gene.alleles));
        }
    });

    it('shows every visible form of every gene in the founders', () => {
        for (const gene of MINI_DRAGON_GENES) {
            const visible = new Set(MINI_FOUNDERS.map((founder) => expressMiniGene(gene.id, founder.genome).id));
            expect(visible.size, gene.id).toBe(gene.forms.length);
        }
    });

    it('hides the baby-bumpy scale rows and teacup size behind carriers', () => {
        const carriesHidden = (geneId: 'coat' | 'size', hidden: string): boolean => MINI_FOUNDERS.some((founder) => founder.genome[geneId].includes(hidden) &&
...
describe('show ring trials', () => {
    it('reads the flight trial off the wing locus alone', () => {
        expect(runMiniTrial('flight', genomeWith({ wings: ['W', 'W'] })).outcome.id).toBe('flight:soars');
        expect(runMiniTrial('flight', genomeWith({ wings: ['W', 'w'] })).outcome.id).toBe('flight:hovers');
        expect(runMiniTrial('flight', genomeWith({ wings: ['w', 'w'] })).outcome.id).toBe('flight:grounded');
    });
    ...
    it('produces a genotype that expresses the requested form, for every form of every gene', () => {
        for (const gene of MINI_DRAGON_GENES) {
            for (const form of gene.forms) {
                ...
                expect(expressMiniGene(gene.id, genome).id, `${gene.id}/${form.id}`).toBe(form.id);
            }
        }
    });
