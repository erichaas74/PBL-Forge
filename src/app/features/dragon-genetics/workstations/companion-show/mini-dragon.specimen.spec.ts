...
describe('mini dragon specimen source', () => {
    ...
    it('reads out every gene as its visible form', () => {
        ...
        for (const gene of MINI_DRAGON_GENES) {
            ...
            expect(readout, gene.id).toBeTruthy();
        }
        expect(readouts.find((entry) => entry.id === 'mini:wings')?.valueLabel).toBe('Small wings');
        expect(readouts.find((entry) => entry.id === 'mini:coat')?.valueLabel).toBe('Baby-bumpy spike rows');
    ...
    it('names the inheritance pattern beside each gene, since that is what differs between them', () => {
        const readouts = miniDragonTraitReadouts(GENOME, 'bench-mini-1');
        expect(readouts.find((entry) => entry.id === 'mini:wings')?.detail).toContain('Incomplete dominance');
        expect(readouts.find((entry) => entry.id === 'mini:pattern')?.detail).toContain('Codominance');
        expect(readouts.find((entry) => entry.id === 'mini:ember')?.detail).toContain('Multiple alleles');
    });
    ...
    it('draws a different individual for a different id on the same genome', () => {
        ...
        const values = (readouts: typeof first) => readouts.filter((entry) => entry.id.startsWith('mini:feature-')).map((e) => e.valueLabel);
        const genes = (readouts: typeof first) => readouts.filter((entry) => !entry.id.startsWith('mini:feature-')).map((e) => e.valueLabel);
