import { bloodlineReport, companionAssembly, founderLinesRepresented, founderToCompanion, kinshipCoefficient, litterConsistency, materializeLitter, meetsStandard, pupToCompanion, rebuildKennel, sameStandard, standardMatches, whelpLitter, } from './companion-show.domain';
...
describe('companion litters', () => {
    ...
    it('rebuilds identical young from the stored record', () => {
        ...
        const replayed = materializeLitter(original.record, founder('mini-biscuit'), founder('mini-cinder'));
...
describe('kennel rebuild', () => {
    it('replays adopted founders and every kept young across generations', () => {
        ...
        const second = whelpLitter(pupToCompanion({ ...first.pups[0], kept: true }), pupToCompanion({ ...first.pups[1], kept: true }), [FLUFFY], 2, 4);

        const rebuilt = rebuildKennel(snapshotWith({
    ...
    it('skips a litter whose parents are no longer in the kennel', () => {
        ...
        const rebuilt = rebuildKennel(snapshotWith({ kennelFounderIds: ['mini-biscuit'], litters: [litter.record] }));
...
describe('bloodline', () => {
    const nodes = new Map([
        ...
        ['a', { id: 'a', generation: 1, parentIds: ['p', 'q'] as readonly [
                    string,
                    string
                ] }],
        ['b', { id: 'b', generation: 1, parentIds: ['p', 'q'] as readonly [
                    string,
                    string
                ] }],
        ['c', { id: 'c', generation: 2, parentIds: ['a', 'b'] as readonly [
                    string,
                    string
                ] }],
    ]);
    ...
    it('flags a sibling pairing and names the shared line', () => {
        ...
        const rebuilt = rebuildKennel(snapshotWith({ kennelFounderIds: ['mini-biscuit', 'mini-pepper'], litters: [record] }));
...
describe('anatomy', () => {
    ...
    it('shortens the legs and keeps the head large on a teacup', () => {
        ...
        const legHeight = (parts: (typeof teacup)['parts']): number => parts.find((part) => part.id === 'mini-leg-front-left')!.dimensions.y;
        const headToBody = (parts: (typeof teacup)['parts']): number => parts.find((part) => part.id === 'mini-head')!.dimensions.y /
    ...
    it('overlaps the slim tail pieces along one continuous centreline', () => {
        ...
        expect(tail2.position.x + tail2.dimensions.x / 2).toBeGreaterThan(tail1.position.x - tail1.dimensions.x / 2);
        expect(plume.position.x + plume.dimensions.x * 0.12).toBeGreaterThan(tail2.position.x - tail2.dimensions.x / 2);
    ...
    it('includes a separate neck and dorsal-scale rows for expression and learned poses', () => {
        ...
        expect(blueprint.parts.find((part) => part.id === 'mini-dorsal-scales')?.roles).toContain('dorsal-scales');
    });
