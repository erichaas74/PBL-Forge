import { FIELD_COLLISION_RADIUS, FieldBounds, FieldEntity, FieldSlot, bounceOffWall, ejectFromSite, pullToSite, respawn, seatInSite, stepField, } from './enzyme-cell-field';
...
describe('enzyme cell field', () => {
    ...
    it('bounces a molecule back off every cell wall', () => {
        ...
        for (const body of [left, right, top, bottom])
            bounceOffWall(body, BOUNDS);
    ...
    it('pulls a captured molecule into the active site and reports arrival', () => {
        ...
        expect(pullToSite(body, 450, 250)).toBe(false);
        for (let frame = 0; frame < 40; frame += 1)
            pullToSite(body, 450, 250);

        expect(pullToSite(body, 450, 250)).toBe(true);
    ...
    it('seats a molecule with no residual drift so the fit reads as exact', () => {
        ...
        expect(body).toEqual(expect.objectContaining({ x: 450, y: 250, rot: 0, vx: 0, vy: 0, vrot: 0, seat: 1 }));
    });
