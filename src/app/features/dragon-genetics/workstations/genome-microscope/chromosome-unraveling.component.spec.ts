...
describe('ChromosomeUnravelingComponent', () => {
    ...
    it('keeps five addressable biological frames in one aligned SVG viewport', () => {
        ...
        expect([...element.querySelectorAll<SVGGElement>('.unravel-frame')].map((frame) => frame.dataset['frame'])).toEqual(['1', '2', '3', '4', '5']);
    ...
    it('lets a student move freely between all unpacking stages', () => {
        ...
        for (let index = 0; index < component.stages.length; index += 1) {
            ...
            expect(element.querySelector('.unravel-frame.active')?.getAttribute('data-frame')).toBe(String(index + 1));
        }
    });
    ...
    it('plays the unpacking sequence in order and stops at exposed DNA', async () => {
        vi.useFakeTimers();
        component.toggleAnimation();
        expect(component.playing()).toBe(true);

        vi.advanceTimersByTime(1250 * 4);
        fixture.detectChanges();
