...
const EXPECTED_RECORDS = DRAGON_GENE_DNA_CATALOG.filter((record) => RELEASED_GENE_IDS.has(record.geneId));
...
describe('ProteinTraitExpressionComponent', () => {
    ...
    it('derives one pathway per released gene from the shared gene catalog', () => {
        expect(component.pathways().map((pathway) => pathway.id)).toEqual(EXPECTED_RECORDS.map((record) => record.geneId));
        expect(component.pathways().map((pathway) => pathway.proteinName)).toEqual(EXPECTED_RECORDS.map((record) => record.protein.traitSignal.name));
        expect(component.pathways().map((pathway) => pathway.shape)).toEqual(EXPECTED_RECORDS.map((record) => record.protein.traitSignal.path));
        expect(component.pathways().every((pathway) => pathway.phenotype.length > 0)).toBe(true);
    });
    ...
    it('carries both routes to a trait: enzyme products and proteins acting directly', () => {
        ...
        expect(component.enzymePathways().length + component.directPathways().length).toBe(component.pathways().length);
    ...
    it('renders each molecule name and its exact generated silhouette in the bank', () => {
        ...
        EXPECTED_RECORDS.forEach((record, index) => {
            ...
            expect(tokens[index].querySelector('.protein-signal')?.getAttribute('d')).toBe(traitSignal.path);
            expect(tokens[index].querySelector('.protein-origin')?.getAttribute('d')).toBe(form.shapePath);
    ...
    it('punches every receptor out of the molecule that opens it', () => {
        ...
        component.pathways().forEach((pathway, index) => {
            ...
            expect(pathway.socketPath.endsWith(pathway.shape)).toBe(true);
            expect(receptors[index].querySelector('.receptor-outline')?.getAttribute('d')).toBe(pathway.shape);
        });
    });
    ...
    it('activates a matching pathway through the select-then-select interaction', () => {
        ...
        expect(component.isActivated(pathway.id)).toBe(true);
