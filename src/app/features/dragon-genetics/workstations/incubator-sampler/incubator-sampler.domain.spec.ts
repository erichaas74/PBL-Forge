import { DRAGON_PARENTS, breedLabClutch, breedLabOffspringProfiles, genotypeLabel, } from '../../simulation/domain/dragon-inheritance';
import { buildIncubatorBatch, buildIncubatorPoolBatch, phenotypeOptions, rebuildIncubatorLineage, } from './incubator-sampler.domain';
...
describe('Incubator Sampler domain', () => {
    ...
    it('builds deterministic visible observations whose bucket counts cover the batch', () => {
        ...
        expect(Math.abs(first.record.results.reduce((sum, result) => sum + result.percentage, 0) - 100)).toBeLessThanOrEqual(1);
        expect(first.record.offspring.every((observation) => ['Winged', 'Wingless'].includes(observation.phenotypeLabel))).toBe(true);
    });
    ...
    it('mixes a bucket pool evenly while hidden inheritance determines visible outcomes', () => {
        ...
        expect(Math.max(...pairCounts.values()) - Math.min(...pairCounts.values())).toBeLessThanOrEqual(1);
    ...
    it('preserves the exact hidden Ww × ww and Ww × Ww Punnett proportions', () => {
        const firstGeneration = buildIncubatorBatch(DRAGON_PARENTS[0], DRAGON_PARENTS[1], 'wings', 1, 21, 8);
        const wingedIds = new Set(firstGeneration.record.results.find((result) => result.label === 'Winged')?.offspringIds);
        const wingedPool = firstGeneration.offspring.filter((dragon) => wingedIds.has(dragon.id));

        expect(wingedPool.length).toBe(4);
        expect(wingedPool.every((dragon) => genotypeLabel(dragon.genome.wings) === 'Ww')).toBe(true);

        const secondGeneration = buildIncubatorPoolBatch(wingedPool, 'wings', 2, 22, 12);
        const hiddenCounts = secondGeneration.offspring.reduce<Record<string, number>>((counts, dragon) => {
        ...
        expect(secondGeneration.record.results.map((result) => [
            result.label,
            result.count,
            result.percentage,
        ])).toEqual([
    ...
    it('rebuilds later-generation parents from persisted offspring IDs', () => {
        const generationOne = buildIncubatorBatch(DRAGON_PARENTS[0], DRAGON_PARENTS[1], 'horns', 1, 7, 8);
        ...
        expect(rebuilt.specimens.has(selected[0].id)).toBe(true);
        expect(rebuilt.batches.get(generationTwo.record.id)?.offspring.length).toBe(4);
        expect(rebuilt.batches.get(generationTwo.record.id)?.record.breedingPoolIds).toEqual(selected.map((dragon) => dragon.id));
    });
});
