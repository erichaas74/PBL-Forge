...
import { DragonHatcheryInstrument, DragonVisualBridge, DragonVisualStageEvent, } from '../../../../shared/dragon-visuals';
...
function stageEvent(type: DragonVisualStageEvent['type'], targetId: string, value?: string | number): DragonVisualStageEvent {
    return {
...
describe('DragonHatcheryStationComponent', () => {
    ...
    it('publishes a valid clutch scene with every egg sealed', () => {
        ...
        expect(instrument().eggs.every((egg) => !egg.examined && !egg.sampled && !egg.hatched)).toBe(true);
    ...
    it('requires a locked prediction before the tools open', () => {
        ...
        expect(station.primaryAction().disabled).toBe(true);

        station.setPrediction(3);
        fixture.detectChanges();
        advance();

        expect(station.predictionLocked()).toBe(true);
    ...
    it('stops candling once the budget is spent', () => {
        ...
        expect(instrument().eggs[1].examined).toBe(false);
        ...
        expect(instrument().eggs[1].sampled).toBe(true);
    });
    ...
    it('hatches only the chosen eggs and compares the result with the prediction', () => {
        ...
        expect(instrument().hatchCommitted).toBe(true);
    ...
    it('saves a record carrying the prediction, the records read, and the pinned evidence', () => {
        ...
        expect(record.evidenceCorrect).toBe(true);
        expect(record.misconception).toBeNull();
        expect(record.eggs.length).toBe(4);
        expect(station.finished()).toBe(true);
    });
    ...
    it('flags the misconception behind an unsupported evidence mark', () => {
        ...
        advance();
        expect(records[0].evidenceCorrect).toBe(false);
