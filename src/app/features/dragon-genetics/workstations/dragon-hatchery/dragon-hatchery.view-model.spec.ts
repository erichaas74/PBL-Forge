import { DRAGON_VISUAL_CONTRACT_VERSION, DragonAnalysisSample, DragonHatcheryInstrument, DragonVisualPhase, DragonVisualScene, } from '../../../../shared/dragon-visuals/domain/dragon-visual.models';
...
function egg(id: string, wings: readonly [
    string,
    string
], fire: readonly [
    string,
    ...
    return {
        ...
        genes: [
            ...
            gene(id, 'fire', 'F', 2, fire, fire.includes('F') ? 'Breathes fire' : 'Does not breathe fire'),
        ],
    };
}

function gene(sampleId: string, traitId: string, geneId: string, chromosomeModel: number, alleles: readonly [
    string,
    string
], phenotypeId: string) {
    return {
...
function sceneWith(phase: DragonVisualPhase, overrides: Partial<DragonHatcheryInstrument> = {}): DragonVisualScene {
    return {
...
describe('Dragon hatchery view model', () => {
    ...
    it('publishes the trait readout after examining, and the allele pair only after sampling', () => {
        const examined = buildDragonHatcheryViewModel(sceneWith('manipulate', {
            eggs: [
                {
                    eggId: 'egg-1',
                    sampleId: 'egg-1',
        ...
        expect(examined?.eggs[0].traits.every((trait) => trait.genotype === null)).toBe(true);
        expect(examined?.eggs[0].genotypeLabel).toBeNull();

        const sampled = buildDragonHatcheryViewModel(sceneWith('manipulate', {
            eggs: [
                {
                    eggId: 'egg-1',
                    sampleId: 'egg-1',
    ...
    it('reveals what a hatched dragon shows without revealing its alleles', () => {
        const model = buildDragonHatcheryViewModel(sceneWith('reveal', {
            eggs: [
                {
                    eggId: 'egg-1',
                    sampleId: 'egg-1',
        ...
        expect(model?.hatchEnabled).toBe(false);
    });
    ...
    it('opens the tools only while manipulating and the hatch only while revealing', () => {
        ...
        expect(observe?.eggs.every((item) => !item.canExamine && !item.canSample)).toBe(true);
        expect(observe?.eggs.every((item) => !item.canStage)).toBe(true);

        const manipulate = buildDragonHatcheryViewModel(sceneWith('manipulate'), COPY);
        expect(manipulate?.eggs.every((item) => item.canExamine && item.canSample)).toBe(true);
        expect(manipulate?.eggs.every((item) => item.canStage)).toBe(true);
        expect(manipulate?.hatchEnabled).toBe(false);

        const reveal = buildDragonHatcheryViewModel(sceneWith('reveal', { selectedEggIds: ['egg-1'] }), COPY);
        expect(reveal?.eggs.every((item) => !item.canExamine)).toBe(true);
        expect(reveal?.hatchEnabled).toBe(true);
    });

    it('closes a tool when its budget is spent and stops staging at the hatch limit', () => {
        const spent = buildDragonHatcheryViewModel(sceneWith('manipulate', { examinesRemaining: 0, samplesRemaining: 1 }), COPY);
        expect(spent?.eggs.every((item) => !item.canExamine)).toBe(true);
        expect(spent?.eggs.every((item) => item.canSample)).toBe(true);
        expect(spent?.tools.find((tool) => tool.id === 'examine')?.remaining).toBe(0);

        const full = buildDragonHatcheryViewModel(sceneWith('manipulate', { selectedEggIds: ['egg-1', 'egg-2'], hatchLimit: 2 }), COPY);
        ...
        expect(full?.eggs.find((item) => item.id === 'egg-1')?.canStage).toBe(true);
        expect(full?.eggs.find((item) => item.id === 'egg-3')?.canStage).toBe(false);
    });

    it('offers only the tools the hosting module enabled', () => {
        const model = buildDragonHatcheryViewModel(sceneWith('manipulate', { availableToolIds: ['examine'] }), COPY);

        expect(model?.tools.map((tool) => tool.id)).toEqual(['examine']);
        expect(model?.eggs.every((item) => item.canExamine)).toBe(true);
        expect(model?.eggs.every((item) => !item.canSample && !item.canStage)).toBe(true);
        expect(model?.stagingEnabled).toBe(false);
    });

    it('locks an egg the module withheld', () => {
        const model = buildDragonHatcheryViewModel(sceneWith('manipulate', {
            eggs: [
                {
                    eggId: 'egg-1',
                    sampleId: 'egg-1',
        ...
        expect(model?.eggs[0].locked).toBe(true);
        expect(model?.eggs[0].canExamine).toBe(false);
        expect(model?.eggs[0].canStage).toBe(false);
