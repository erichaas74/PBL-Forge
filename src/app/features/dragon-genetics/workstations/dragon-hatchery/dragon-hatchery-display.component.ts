...
import { ChromosomePairComponent } from '../../../../shared/dragon-visuals/displays/shared/chromosome-pair.component';
import {
  StationCopy,
  resolveStationCopy,
} from '../../../../shared/dragon-visuals/displays/shared/station-copy';
import { createReducedMotionSignal } from '../../../../shared/dragon-visuals/displays/shared/reduced-motion';
import { createTeachingSequencePlayer } from '../../../../shared/dragon-visuals/displays/shared/teaching-sequence.player';
...
export class DragonHatcheryDisplayComponent {
  ...
  readonly reducedMotion = computed(
    () => this.reducedMotionOverride() ?? this.systemReducedMotion(),
  );
  ...
  commitHatch(): void {
    ...
    if (!model || (!model.hatchEnabled && !this.openLabHatchEnabled(model))) return;
    this.emit('hatch-committed', model.targets.hatchControl, model.stagedEggs.length);
  }

  openLabHatchEnabled(
    model: NonNullable<ReturnType<typeof buildDragonHatcheryViewModel>>,
  ): boolean {
    return (
      this.openLab() &&
      model.phase === 'manipulate' &&
      !model.hatchCommitted &&
      model.stagedEggs.length > 0
    );
  }
