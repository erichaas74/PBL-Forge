...
import {
  MEIOSIS_PHASE_COUNT,
  meiosisGameteCellChromosomes,
  meiosisStageView,
} from './meiosis-cell-stage';
...
describe('meiosis cell stage', () => {
  ...
  it('opens and closes on the whole cell, and zooms to the nucleus for every division', () => {
    const focusByPhase = Array.from(
      { length: MEIOSIS_PHASE_COUNT },
      (_, phase) => meiosisStageView(run, phase).focus,
    );
  ...
  it('starts as one diploid cell of ten unreplicated chromosomes', () => {
    ...
    expect(view.cells[0].chromosomes.every((item) => !item.pairedModel)).toBeTrue();
  });
  ...
  it('replicates every chromosome into two identical sister chromatids at S phase', () => {
    ...
    view.cells[0].chromosomes.forEach((item) => {
      ...
      expect(item.recombinant).toBeFalse();
    });
  });
  ...
  it('shows no recombination before the pairs cross over in prophase I', () => {
    [0, 1].forEach((phase) => {
      expect(
        meiosisStageView(run, phase).cells[0].chromosomes.some((item) => item.recombinant),
      ).toBeFalse();
    });

    expect(meiosisStageView(run, 2).cells[0].chromosomes.some((item) => item.recombinant)).toBeTrue();
  });
  ...
  it('holds five homologous pairs together through prophase I', () => {
    ...
    expect(view.cells[0].chromosomes.some((item) => item.recombinant)).toBeTrue();
  });
  ...
  it('sends each daughter cell exactly the chromosomes its two gametes keep', () => {
    ...
    expect(separating.slice(0, 5).map((item) => item.id)).toEqual(
      daughters[0].chromosomes.map((item) => item.id),
    );
    expect(separating.slice(5).map((item) => item.id)).toEqual(
      daughters[1].chromosomes.map((item) => item.id),
    );
  });
  ...
  it('reforms two nuclei at telophase I without moving the chromosomes again', () => {
    ...
    expect(telophase.chromosomes.map((item) => item.id)).toEqual(
      anaphase.chromosomes.map((item) => item.id),
    );
  });
  ...
  it('separates in anaphase II exactly what each gamete ends up carrying', () => {
    ...
    expect(firstCell.slice(0, 5).map((item) => item.model.loci)).toEqual(
      gameteOne.map((item) => item.model.loci),
    );
  });
