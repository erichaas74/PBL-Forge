...
describe('DragonHatcheryDisplayComponent', () => {
  ...
  it('stages an egg and commits the hatch through semantic events', () => {
    ...
    expect(
      host().querySelector<HTMLButtonElement>('[data-target="hatch-control"]')?.disabled,
    ).toBeTrue();
  ...
  it('selects an egg for the bench and publishes a live summary', () => {
    ...
    expect(host().querySelector<HTMLElement>('.bay')?.style.getPropertyValue('--dh-brass')).toBe(
      DRAGON_HATCHERY_THEME.palette.brass,
    );
    expect(host().querySelector('[aria-live="polite"]')?.textContent).toContain(
      'Clutch of 2 eggs: 0 examined, 0 sampled, 0 hatched.',
    );
  });
