...
describe('meiosis gamete domain', () => {
  ...
  it('models five pairs, physical crossovers, and four distinct haploid products', () => {
    ...
    expect(run.gametes.every((gamete) => gamete.chromosomes.length === 5)).toBeTrue();

    const signatures = run.gametes.map((gamete) =>
      Object.entries(gamete.alleleByTrait).sort().map(([trait, allele]) => `${trait}:${allele}`).join('|'),
    );
  ...
  it('keeps homologs together through Meiosis I before separating sisters in Meiosis II', () => {
    ...
    for (const pair of run.chromosomePairs) {
      const origins = run.gametes.map((gamete) => {
        const product = gamete.chromosomes.find(
          (chromosome) => chromosome.chromosome === pair.chromosome,
        )!;
        return pair.chromatids.find(
          (chromatid) => chromatid.id === product.sourceChromatidId,
        )!.origin;
      });
  ...
  it('moves the allele-bearing suffix between non-sister chromatids at a crossover', () => {
    ...
    for (const pair of run.chromosomePairs.filter(
      (candidate) => candidate.crossoverAfterLocusIndex !== null,
    )) {
      const split = pair.crossoverAfterLocusIndex!;
      const aRecombinant = pair.chromatids[1];
      const bRecombinant = pair.chromatids[2];
      expect(aRecombinant.loci.slice(0, split).every((locus) => locus.origin === 'homolog-a'))
        .toBeTrue();
      expect(aRecombinant.loci.slice(split).every((locus) => locus.origin === 'homolog-b'))
        .toBeTrue();
      expect(bRecombinant.loci.slice(0, split).every((locus) => locus.origin === 'homolog-b'))
        .toBeTrue();
      expect(bRecombinant.loci.slice(split).every((locus) => locus.origin === 'homolog-a'))
        .toBeTrue();
    }
  });
  ...
  it('produces two X-bearing and two Y-bearing products for a male parent', () => {
    ...
    const sexChromosomes = run.gametes.map((gamete) =>
      gamete.chromosomes.find((chromosome) => chromosome.chromosome === 'Chr X')?.sexChromosome,
    );
