...
import { HatcheryEvidenceOption } from './dragon-hatchery.models';
...
export const HATCHERY_STATION_COPY: StationCopy = {
  'hatchery.read-clutch':
    'This clutch came from one cross. Every egg carries a full genome already.',
  'hatchery.predict': 'Predict before you open anything. Lock a number, then start examining.',
  'hatchery.examine':
    'Candling reads the traits an egg will show. It reports nothing about the alleles.',
  'hatchery.sample':
    'A DNA sample reads the allele pair. Two eggs that look alike can differ here.',
...
export function hatcheryClutchCopy(input: {
  ...
  return {
    ...
    ...Object.fromEntries(
      (input.evidence ?? DEFAULT_HATCHERY_EVIDENCE).map((mark) => [
        `evidence.${mark.id}`,
        mark.text,
      ]),
    ),
