...
import { DRAGON_DNA_BASE_COLORS, DRAGON_ENZYME_GENES, DRAGON_GENE_DNA_CATALOG, DragonDnaBase, geneAlleleMarking, geneDnaRecord, geneMutationTypeForComparison, geneProteinForm, } from './dragon-gene-dna.catalog';

describe('dragon gene DNA catalog', () => {
    it('defines molecular and visual truth for every identified gene', () => {
        expect(DRAGON_GENE_DNA_CATALOG.map((record) => record.geneId).sort()).toEqual(EXPRESSIVE_DRAGON_TRAITS.map((trait) => trait.id).sort());
        expect(new Set(DRAGON_GENE_DNA_CATALOG.map((record) => record.alleleASequence)).size).toBe(DRAGON_GENE_DNA_CATALOG.length);
    });
    ...
    it('keeps sequence, mutation, barcode bases, and base colors aligned', () => {
        ...
        for (const record of DRAGON_GENE_DNA_CATALOG) {
            ...
            expect(record.alleles[1].barcode.length).toBe(record.mutation.type === 'deletion' ? 5 : record.mutation.type === 'insertion' ? 7 : 6);
    ...
    it('gives every reference gene eight complete amino-acid codons', () => {
        ...
        for (const record of DRAGON_GENE_DNA_CATALOG) {
            ...
            expect(codons.some((codon) => stopCodons.has(codon))).toBe(false);
        }
    });

    it('contains substitution, deletion, and insertion genes', () => {
        expect(new Set(DRAGON_GENE_DNA_CATALOG.map((record) => record.mutation.type))).toEqual(new Set(['substitution', 'deletion', 'insertion']));
    });
    ...
    it('punches every receptor out of the molecule that opens it', () => {
        for (const record of DRAGON_GENE_DNA_CATALOG) {
            ...
            expect(traitSocketPath.startsWith('M 0 0 H 160 V 120 H 0 Z')).toBe(true);
            expect(traitSocketPath.endsWith(traitSignal.path)).toBe(true);
        }
    });
    ...
    it('splits genes into enzymes and proteins that reach a trait directly', () => {
        ...
        expect(DRAGON_ENZYME_GENES.map((record) => record.geneId)).toEqual(enzymes.map((record) => record.geneId));
    ...
    it('runs enzymes in both directions and hands each trait one product', () => {
        ...
        for (const record of DRAGON_ENZYME_GENES) {
            ...
            expect(activity.fragmentA.path.startsWith('M 0 0 H 80')).toBe(true);
            expect(activity.fragmentB.path.startsWith('M 80 0 H 160')).toBe(true);
        }
    });
