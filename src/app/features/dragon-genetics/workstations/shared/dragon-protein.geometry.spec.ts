import { MOLECULE_BOX, PROTEIN_CONNECTOR_TYPES, enzymeShapeSet, foldedProteinPath, noise, residueChainPath, socketPlatePath, } from './dragon-protein.geometry';
...
describe('dragon protein geometry', () => {
    ...
    it('splits the product into a left and a right substrate on one connector', () => {
        for (const connector of PROTEIN_CONNECTOR_TYPES) {
            ...
            expect(left.every((point) => point.x <= 108)).toBe(true);
            expect(right.every((point) => point.x >= 52)).toBe(true);
    ...
    it('punches a receptor socket that is the exact negative of its molecule', () => {
        ...
        expect(socket.startsWith(`M 0 0 H ${MOLECULE_BOX.width} V ${MOLECULE_BOX.height} H 0 Z`)).toBe(true);
        expect(socket.endsWith(set.product)).toBe(true);
    ...
    it('returns an open backbone for the residue chain and nothing for an empty one', () => {
        ...
        expect(backbone.startsWith('M')).toBe(true);
