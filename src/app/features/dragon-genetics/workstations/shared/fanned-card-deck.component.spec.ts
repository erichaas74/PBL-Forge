...
class FannedDeckTestHost {
  items: readonly FannedDeckItem[] = [{ id: 'a' }, { id: 'b' }, { id: 'c' }, { id: 'd' }];
  activeId = 'b';
  readonly labelFor = (item: FannedDeckItem) => `Dragon ${item.id.toUpperCase()}`;

  @ViewChild(FannedCardDeckComponent) deck!: FannedCardDeckComponent;
...
describe('FannedCardDeckComponent', () => {
  ...
  it('centres one card and exposes the previous and next names', () => {
    ...
    expect(element.querySelector('.is-previous .fanned-deck__peek')?.textContent).toContain(
      'Dragon A',
    );
  });

  it('brings an exposed card forward and wraps arrow navigation', () => {
  ...
  it('keeps the centred stack for decks of up to ten cards', () => {
    host.items = Array.from({ length: 10 }, (_, index) => ({ id: `dragon-${index}` }));
    host.activeId = 'dragon-5';
    fixture.detectChanges();

    const element = fixture.nativeElement as HTMLElement;
    expect(element.querySelectorAll('.fanned-deck__slot')).toHaveSize(10);
