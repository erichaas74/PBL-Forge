import { CELL_ANNOTATIONS, CELL_BODY, CELL_ER_RIBBONS, CELL_NUCLEAR_PORES, CELL_NUCLEOLUS, CELL_NUCLEUS, CELL_ORGANELLES, CELL_POLES, CELL_RIBOSOMES, CELL_SPINDLE_POLES, CELL_VIEW, cellFocusRect, cellMembranePath, cellNucleusPath, cellViewBox, chromosomeSlots, insideEllipse, metaphasePlateSlots, polarSlots, slotInsideEllipse, } from './cell-model.geometry';
...
describe('cell model geometry', () => {
    it('keeps every chromosome slot inside the nucleus at any count', () => {
        for (let count = 1; count <= 24; count += 1) {
            ...
            slots.forEach((slot) => {
                ...
                expect(slotInsideEllipse(slot, CELL_NUCLEUS), `count ${count}, slot at ${slot.x},${slot.y} sized ${slot.width}`).toBe(true);
            });
        }
    });
    ...
    it('keeps replicated chromosomes inside the nucleus despite their extra height', () => {
        ...
        slots.forEach((slot) => {
            ...
            expect(slotInsideEllipse(slot, CELL_NUCLEUS)).toBe(true);
        });
    });
        ...
        layouts.forEach((slots, layout) => {
            slots.forEach((slot, index) => {
                slots.slice(index + 1).forEach((other) => {
                    const apart = Math.abs(slot.x - other.x) >= (slot.width + other.width) / 2 ||
                        Math.abs(slot.y - other.y) >= (slot.height + other.height) / 2;
                    expect(apart, `layout ${layout}: slots ${index} and ${index + 1} overlap`).toBe(true);
                });
            });
        });
    });
    ...
    it('stacks the metaphase plate down the middle when the poles are at the sides', () => {
        ...
        slots.forEach((slot) => expect(slotInsideEllipse(slot, CELL_BODY)).toBe(true));
    });
    ...
    it('lays the metaphase plate across the middle when the poles are top and bottom', () => {
        ...
        expect(slots.every((slot) => slot.width > slot.height)).toBe(true);
        slots.forEach((slot) => expect(slotInsideEllipse(slot, CELL_BODY)).toBe(true));
    });
    ...
    it('splits chromosomes between the two poles on either division axis', () => {
        ...
        expect(sideways.slice(0, 3).every((slot) => slotInsideEllipse(slot, CELL_POLES.horizontal.a))).toBe(true);
        expect(sideways.slice(3).every((slot) => slotInsideEllipse(slot, CELL_POLES.horizontal.b))).toBe(true);
        expect(sideways.every((slot) => slotInsideEllipse(slot, CELL_BODY))).toBe(true);

        const endways = polarSlots(10, CHROMOSOME_RATIO, 'vertical');

        expect(endways.slice(0, 5).every((slot) => slot.y < CELL_BODY.cy)).toBe(true);
        expect(endways.slice(5).every((slot) => slot.y > CELL_BODY.cy)).toBe(true);
        expect(endways.slice(0, 5).every((slot) => slotInsideEllipse(slot, CELL_POLES.vertical.a))).toBe(true);
        expect(endways.slice(5).every((slot) => slotInsideEllipse(slot, CELL_POLES.vertical.b))).toBe(true);
        expect(endways.every((slot) => slotInsideEllipse(slot, CELL_BODY))).toBe(true);
    });
    ...
    it('puts the spindle poles square across whichever way the cell divides', () => {
        ...
        [CELL_SPINDLE_POLES.horizontal, CELL_SPINDLE_POLES.vertical].forEach((poles) => {
            expect(insideEllipse(poles.a, CELL_BODY)).toBe(true);
            expect(insideEllipse(poles.b, CELL_BODY)).toBe(true);
        });
    });
    ...
    it('places every organelle in the cytoplasm, never in the nucleus or outside the cell', () => {
        ...
        CELL_ORGANELLES.forEach((organelle) => {
            ...
            expect(insideEllipse(point, CELL_BODY, 0.92), `${organelle.id} escaped the membrane`).toBe(true);
            expect(insideEllipse(point, CELL_NUCLEUS, 1.05), `${organelle.id} landed inside the nucleus`).toBe(false);
        });
    });
    ...
    it('scatters free ribosomes through the cytoplasm only', () => {
        ...
        CELL_RIBOSOMES.forEach((point) => {
            expect(insideEllipse(point, CELL_BODY, 0.9)).toBe(true);
            expect(insideEllipse(point, CELL_NUCLEUS, 1.14)).toBe(false);
        });
    });
    ...
    it('wraps the rough ER around the nucleus without crossing the membrane', () => {
        ...
        CELL_ER_RIBBONS.forEach((ribbon) => {
            ...
            ribbon.ribosomes.forEach((point) => {
                expect(insideEllipse(point, CELL_BODY, 0.94)).toBe(true);
            });
        });
    });

    it('keeps the nucleolus and the nuclear pores on the nucleus', () => {
        expect(insideEllipse({ x: CELL_NUCLEOLUS.cx, y: CELL_NUCLEOLUS.cy }, CELL_NUCLEUS)).toBe(true);
        expect(CELL_NUCLEAR_PORES.length).toBe(18);
        CELL_NUCLEAR_PORES.forEach((pore) => {
            expect(insideEllipse(pore, CELL_NUCLEUS, 1.001)).toBe(true);
            expect(insideEllipse(pore, CELL_NUCLEUS, 0.999)).toBe(false);
        });
    });
    ...
    it('anchors every annotation on the cell and keeps its text in the label gutter', () => {
        ...
        CELL_ANNOTATIONS.forEach((annotation) => {
            ...
            expect(insideEllipse(annotation.target, CELL_BODY), `${annotation.id} points outside the cell`).toBe(true);
            ...
            const clear = annotation.anchor === 'start'
                ? annotation.from.x >= annotation.x + textWidth || annotation.from.x <= annotation.x - 1
                : annotation.from.x <= annotation.x - textWidth || annotation.from.x >= annotation.x + 1;
            expect(clear, `${annotation.id} leader crosses its own text`).toBe(true);
        });
    });
    ...
    it('draws a closed membrane that pinches into a furrow as the cell divides', () => {
        ...
        expect(resting.startsWith('M ')).toBe(true);
        expect(resting.endsWith(' Z')).toBe(true);
        expect(dividing).not.toBe(resting);
        expect(cellMembranePath(0.5, 0.96)).not.toBe(resting);
        expect(cellNucleusPath().endsWith(' Z')).toBe(true);
    });
    ...
    it('pulls the frame back once the chromosomes leave for the poles', () => {
        ...
        expect(cellFocusRect(polarSlots(10, CHROMOSOME_RATIO, 'vertical')).height).toBeGreaterThan(held.height);
