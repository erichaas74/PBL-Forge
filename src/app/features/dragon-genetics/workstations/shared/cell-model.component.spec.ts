...
describe('CellModelComponent', () => {
  ...
  it('draws the organelles a cell diagram is expected to show', () => {
    const kinds = [...element.querySelectorAll('[data-organelle]')].map((node) =>
      node.getAttribute('data-organelle'),
    );

    expect(new Set(kinds)).toEqual(
      new Set(['mitochondrion', 'golgi', 'lysosome', 'vacuole', 'smooth-er', 'centrosome']),
    );
  ...
  it('does not emit a selection from a read-only cell', () => {
    ...
    expect(element.querySelector<HTMLButtonElement>('[data-chromosome="Chr 1"]')?.disabled).toBeTrue();
  });

  it('names each chromosome and the cell for assistive technology', () => {
    expect(element.querySelector('[data-chromosome="Chr 1"]')?.getAttribute('aria-label')).toContain(
      'single chromosome',
    );
  ...
  it('gives replicated chromosomes taller slots so they still fit the nucleus', () => {
    ...
    expect(joined.every((slot, index) => slot.height > single[index])).toBeTrue();
  ...
  it('labels the drawn structures only when the diagram asks for it', () => {
    ...
    const labels = [...element.querySelectorAll('.cell-model__annotation-text')].map((node) =>
  ...
  it('shows an empty cell rather than a broken one when nothing is loaded', () => {
    ...
    expect(element.querySelector('.cell-model__empty')?.textContent).toContain(
      'No chromosomes available',
    );
    expect(element.querySelector('.cell-model__membrane')).not.toBeNull();
  });
});
