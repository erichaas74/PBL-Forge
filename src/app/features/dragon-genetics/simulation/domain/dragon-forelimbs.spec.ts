import { AssemblyBlueprint, AssemblyPart, Vector3Data, } from '../../../../shared/assembly/domain/assembly.models';
import { identityQuaternion, rotateVectorByQuaternion, } from '../../../../shared/assembly/domain/vector-data';
import { DRAGON_REARED_POSE, DRAGON_RESTING_POSE, dragonRestingPose, hasGraspingForelimbs, } from '../../../../shared/assembly/preview/specimen-stance';
import { buildSpecimenPose } from '../../../../shared/assembly/preview/specimen-pose';
import { DEFAULT_EXPRESSIVE_DRAGON, ExpressiveDragonProfile, } from './dragon-expressive-genome';
...
function frontLimbs(blueprint: AssemblyBlueprint): AssemblyPart[] {
    return blueprint.parts.filter(part => part.id.includes('front') && part.roles?.includes('leg'));
}
...
describe('dragon forelimb body plans', () => {
    ...
    it('sizes the enlarged grasping chain and its mass consistently', () => {
        for (const arm of frontLimbs(grasping)) {
            ...
            expect(arm.dimensions.y, arm.id).toBeCloseTo(leg.dimensions.y * factor);
            expect(arm.mass, arm.id).toBeCloseTo(leg.mass * factor ** 3);
        }
    });
    ...
    it('keeps the rescaled chain connected at its joints', () => {
        ...
        for (const joint of grasping.joints) {
            ...
            if (!parent || !child || !child.id.includes('front'))
                continue;
            ...
            for (const axis of ['x', 'y', 'z'] as const) {
                expect(onChild[axis], `${joint.childPartId}.${axis}`).toBeCloseTo(onParent[axis], 5);
            }
        }
    });

    it('hangs the arms higher on the torso than the legs they replaced', () => {
        const shoulder = grasping.joints.find(joint => joint.childPartId.includes('front') && !joint.parentPartId.includes('front'))!;
        const hip = walking.joints.find(joint => joint.childPartId.includes('front') && !joint.parentPartId.includes('front'))!;
