...
describe('project hub selectors', () => {
  ...
  it('prioritizes required revision work over another activity in progress', () => {
    ...
    expect(view.nextAction).toEqual(
      jasmine.objectContaining({ activityId: 'observe', reason: 'needs-revision' }),
    );
    expect(view.stages.flatMap((stage) => stage.activities).filter((item) => item.isNextAction))
      .withContext('only one activity should receive primary emphasis')
      .toHaveSize(1);
  });
  ...
  it('keeps optional work out of required progress and offers it after required work', () => {
    ...
    expect(view.nextAction).toEqual(
      jasmine.objectContaining({ activityId: 'extension', reason: 'extension' }),
    );
  });
  ...
  it('rejects a project definition with an unknown prerequisite', () => {
    ...
    expect(() => buildProjectHubViewModel(invalid, ASSIGNMENT, emptyStudentState())).toThrowError(
  ...
  it('offers one path choice after core work and counts only the selected path', () => {
    const project: ProjectHubDefinition = {
      ...
      activities: PROJECT.activities.map((activity) =>
        activity.id === 'extension' ? { ...activity, required: true } : activity,
      ),
      paths: [
        {
          id: 'arena',
    ...
    expect(choiceView.nextAction).toEqual(
    ...
    expect(selectedView.nextAction).toEqual(
      jasmine.objectContaining({ kind: 'activity', activityId: 'extension', reason: 'start' }),
    );
    expect(selectedView.paths[0].selected).toBeTrue();
  });
});
...
function progress(
  activityId: string,
  status: 'in-progress' | 'submitted' | 'needs-revision' | 'complete',
) {
  return {
    activityId,
    status,
