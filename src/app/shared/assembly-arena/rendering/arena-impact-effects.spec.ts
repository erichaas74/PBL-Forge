...
describe('ArenaImpactEffects', () => {
  ...
  it('settles back to a still camera', () => {
    ...
    for (let frame = 0; frame < 200; frame += 1) effects.update(1 / 60);
  ...
  it('shows a wind-up ring and clears it', () => {
    ...
    expect(scene.getObjectByName(TELEGRAPH_MESH_NAME)?.visible).toBeTrue();

    effects.setTelegraph('red', at(2, 1, 3), 0);
    expect(scene.getObjectByName(TELEGRAPH_MESH_NAME)?.visible).toBeFalse();
  });
