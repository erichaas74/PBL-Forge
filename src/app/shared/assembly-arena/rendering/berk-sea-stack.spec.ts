...
import {
  SEA_STACK_CLIFF_HEIGHT,
  createSeaStackIsland,
  seaStackFooting,
} from './berk-sea-stack';
...
describe('berk sea stack', () => {
  ...
  it('hangs the seaward side of the stand off the rock', () => {
    expect(isOverWater(deckRadius, 0))
      .withContext('the front of the gallery should be over open water')
      .toBe(true);
    expect(isOverWater(pitRadius, 0))
      .withContext('the front of the sand should be over open water too')
      .toBe(true);
  });

  it('leaves the back of the ring and both gatehouses on solid ground', () => {
    expect(isOverWater(-deckRadius, 0))
      .withContext('the back of the gallery is the part that rests on rock')
      .toBe(false);
    ...
    for (const side of [-1, 1]) {
      expect(isOverWater(0, side * (pitRadius + 0.44)))
        .withContext(`gatehouse at z=${side * (pitRadius + 0.44)}`)
        .toBe(false);
    }
  });
  ...
  it('builds an island that reaches from the stand down past the waterline', () => {
    ...
    expect(bounds.min.y)
      .withContext('the column should carry on below the sea, not stop at it')
      .toBeLessThan(-SEA_STACK_CLIFF_HEIGHT);
    ...
    expect(highestUnderTheArena(island.group, deckRadius + 4))
      .withContext('something under the stand is poking up into the ring')
      .toBeLessThan(4);
    ...
    expect(surf).withContext('expected a foam collar').toBeDefined();
    island.update(0.4);
...
function highestUnderTheArena(root: THREE.Object3D, range: number): number {
  ...
  root.traverse((object) => {
    ...
    if (Math.hypot(center.x, center.z) > range) return;
...
function firstTransparentMesh(root: THREE.Object3D): THREE.Mesh | undefined {
  ...
  root.traverse((object) => {
    ...
    if (!Array.isArray(material) && material.transparent) found = object;
  });
  return found;
}
