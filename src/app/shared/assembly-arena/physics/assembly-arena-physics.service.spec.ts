...
import { createFounderDragonGenome, generateDragonAssembly, } from '../../../features/dragon-genetics/simulation/domain/dragon-phenotype-builder';
...
import { coreHalfExtents, createCombatant, createPartStatuses, dragonBodyFrame, } from '../utils/battle-assembly';
...
describe('AssemblyArenaPhysicsService', () => {
    it('keeps battling dragons assembled when a duel begins', () => {
        ...
        const combatant = createCombatant('red-1', asset, 'red', setup.redSpawn, 'player', 'dragon-attack', setup.redInitialRotation);
        const opponent = createCombatant('blue-1', asset, 'blue', setup.blueSpawn, 'ai', 'dragon-attack', setup.blueInitialRotation);
        ...
        expect(brokenPartIds, brokenPartIds.join(', ')).toEqual([]);
        for (const dragon of state.combatants) {
            const core = service.getSnapshots().find(snapshot => snapshot.bodyKey === `${dragon.id}:${dragon.corePartId}`);
            expect(core).toBeDefined();
            const up = rotateVectorByQuaternion({ x: 0, y: 1, z: 0 }, core!.quaternion);
            expect(up.y, `${dragon.id} ended the opening exchange overturned`).toBeGreaterThan(0.5);
        }
    });
    ...
    it('self-rights a dragon that has been knocked upside down', () => {
        ...
        const setup = getArenaSetup('duel-arena');
        const combatant = createCombatant('red-1', asset, 'red', { x: setup.redSpawn.x, y: 4, z: setup.redSpawn.z }, 'player', 'dragon-attack', { x: Math.PI, y: 0, z: 0 });
        const state: BattleArenaState = {
        ...
        const core = service.getSnapshots().find(snapshot => snapshot.bodyKey === `red-1:${combatant.corePartId}`);
        expect(core).toBeDefined();
        const up = rotateVectorByQuaternion({ x: 0, y: 1, z: 0 }, core!.quaternion);
    ...
    it('moves the torso forward without using leg contact for locomotion', () => {
        ...
        const horizontalTravel = Math.hypot(finish.position.x - start.x, finish.position.z - start.z);
        expect(horizontalTravel, `start=${JSON.stringify(start)} finish=${JSON.stringify(finish.position)}`).toBeGreaterThan(2);
    ...
    it('slides a dragon off an opponent it has been dropped on top of', () => {
        ...
        const bottom = createCombatant('red-1', asset, 'red', { x: 0, y: 0, z: 0 }, 'player', 'dragon-attack', { x: 0, y: 0, z: 0 });
        const top = createCombatant('blue-1', asset, 'blue', { x: 0, y: core.dimensions.y * 1.2, z: 0 }, 'player', 'dragon-attack', { x: 0, y: 0, z: 0 });
        ...
        const apart = Math.hypot(topCore.position.x - bottomCore.position.x, topCore.position.z - bottomCore.position.z);

        expect(rise, `still stacked: bottom=${JSON.stringify(bottomCore.position)} top=${JSON.stringify(topCore.position)}`).toBeLessThan(core.dimensions.y);
        ...
        expect(apart, `bottom=${JSON.stringify(bottomCore.position)} top=${JSON.stringify(topCore.position)}`).toBeGreaterThan(core.dimensions.z / 2);
    ...
    it('shakes off a rider that keeps driving back on top', () => {
        ...
        const core = asset.assembly.parts.find(part => part.id === 'classic-dragon-body')!;
        const bottom = createCombatant('red-1', asset, 'red', { x: 0, y: 0, z: 0 }, 'player', 'dragon-attack', { x: 0, y: 0, z: 0 });
        const top = createCombatant('blue-1', asset, 'blue', { x: 0, y: core.dimensions.y * 1.2, z: 0 }, 'ai', 'dragon-attack', { x: 0, y: 0, z: 0 });
        ...
        expect(longestRide / 60, `pinned for ${(longestRide / 60).toFixed(2)}s of a driven re-mount`).toBeLessThan(2);
    });
    ...
    it('does not let a held boost turn into a hover', () => {
        ...
        expect(peak, 'boost never left the ground').toBeGreaterThan(standing);
        expect(peak, `climbed to ${peak} on held boost`).toBeLessThan(standing * 4);
    });
    ...
    it('absorbs most of a hit for a guarding dragon and none for a rolling one', () => {
        ...
        const setup = getArenaSetup('duel-arena');
        const attacker = createCombatant('red-1', dragonAsset, 'red', { x: 0, y: 0, z: 0 }, 'player', 'dragon-attack', { x: 0, y: 0, z: 0 });
        const defender = createCombatant('blue-1', targetAsset, 'blue', { x: 2.2, y: 1.32, z: 0 }, 'player', 'dragon-attack', { x: 0, y: 0, z: 0 });
        ...
        expect(open, 'the bite never landed at all').toBeGreaterThan(0);
    ...
    it('knocks a charged dragon down and takes its controls away', () => {
        ...
        const setup = getArenaSetup('duel-arena');
        const attacker = createCombatant('red-1', dragonAsset, 'red', { x: 0, y: 0, z: 0 }, 'player', 'dragon-attack', { x: 0, y: 0, z: 0 });
        const defender = createCombatant('blue-1', dragonAsset, 'blue', { x: 3, y: 0, z: 0 }, 'player', 'dragon-attack', { x: 0, y: Math.PI, z: 0 });
        ...
        expect(knockedDown, 'the charge landed without putting anyone down').toBe(true);
    });
    ...
    it('holds each move to its own cooldown', () => {
        ...
        const setup = getArenaSetup('duel-arena');
        const attacker = createCombatant('red-1', dragonAsset, 'red', { x: 0, y: 0, z: 0 }, 'player', 'dragon-attack', { x: 0, y: 0, z: 0 });
        const target = createCombatant('blue-1', targetAsset, 'blue', { x: 2.2, y: 1.32, z: 0 }, 'static', 'static-target', { x: 0, y: 0, z: 0 });
        ...
        expect(charges, `${charges} charges landed in six seconds`).toBeLessThanOrEqual(3);
    });
    ...
    it('runs a timed bite pose and hit without requiring limb contact', () => {
        ...
        const targetAsset = createTargetAsset();
        const setup = getArenaSetup('duel-arena');
        const dragon = createCombatant('red-1', dragonAsset, 'red', { x: 0, y: 0, z: 0 }, 'player', 'dragon-attack', { x: 0, y: 0, z: 0 });
        ...
        for (let frame = 0; frame < 60; frame += 1) {
            ...
            strongestTargetReaction = Math.max(strongestTargetReaction, Math.hypot(targetSnapshot.velocity.x, targetSnapshot.velocity.z));
        }
    ...
    it('stands a dragon on the sand rather than through it', () => {
        ...
        const dragon = createCombatant('red-1', asset, 'red', { x: 0, y: 0, z: 0 }, 'player', 'dragon-attack', { x: 0, y: 0, z: 0 });
        ...
        expect(soles, `soles at ${soles.toFixed(3)}`).toBeGreaterThan(-0.05);
        // And not hovering either: a dragon lifted clear of its own shadow would be
        // just as wrong as one buried in the sand.
        expect(soles, `soles at ${soles.toFixed(3)}`).toBeLessThan(0.25);
    });
    ...
    it('keeps a dragon running at the palisade inside the ring', () => {
        ...
        for (let frame = 0; frame < 60 * 12; frame += 1) {
            ...
            furthest = Math.max(furthest, furthestDrawnPart(asset.assembly, dragon, coreSnapshot(service, dragon)));
        }

        expect(furthest, `reached ${furthest.toFixed(2)} of a ${pitRadius} ring`).toBeLessThanOrEqual(pitRadius);
...
function lowestDrawnPoint(blueprint: Parameters<typeof buildDragonArenaPose>[1], combatant: ReturnType<typeof createCombatant>, core: ReturnType<typeof coreSnapshot>): number {
    const parts = new Map(blueprint.parts.map(part => [part.id, part]));
    return Math.min(...buildDragonArenaPose(combatant.id, blueprint, combatant.corePartId, core, undefined).map(posed => {
...
function furthestDrawnPart(blueprint: Parameters<typeof buildDragonArenaPose>[1], combatant: ReturnType<typeof createCombatant>, core: ReturnType<typeof coreSnapshot>): number {
    return Math.max(...buildDragonArenaPose(combatant.id, blueprint, combatant.corePartId, core, undefined).map(posed => Math.hypot(posed.position.x, posed.position.z)));
}

function createTestState(setup: ReturnType<typeof getArenaSetup>, combatants: ReturnType<typeof createCombatant>[], matchId: number): BattleArenaState {
    return {
        combatants,
...
function coreSnapshot(service: AssemblyArenaPhysicsService, combatant: ReturnType<typeof createCombatant>) {
    return service.getSnapshots().find(snapshot => snapshot.bodyKey === `${combatant.id}:${combatant.corePartId}`)!;
}
