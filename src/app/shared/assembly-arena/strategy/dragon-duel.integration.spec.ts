...
import { BattleArenaState, MoveLicenseByCombatant, } from '../models/arena.models';
...
function runDuel(licenses: MoveLicenseByCombatant, seconds = 20): DuelResult {
    ...
    const player = createCombatant('red-1', asset, 'red', setup.redSpawn, 'player', 'dragon-attack', setup.redInitialRotation);
    const challenger = createCombatant('blue-1', asset, 'blue', setup.blueSpawn, 'ai', 'dragon-attack', setup.blueInitialRotation);
    ...
    for (let frame = 0; frame < 60 * seconds; frame += 1) {
        ...
        // challenger's doing.
        const frames = buildControlFrames(state, snapshots, programs, { ...NEUTRAL_CONTROL_FRAME }, {}, { awareness: physics.getCombatAwareness(state), licenses });
        ...
        result.closestGap = Math.min(result.closestGap, Math.hypot(blue.position.x - red.position.x, blue.position.z - red.position.z));
        /*
...
describe('dragon duel', () => {
    it('lands real attacks on a player who is standing still', () => {
        ...
        expect(duel.closestGap, 'the challenger never even closed the distance').toBeLessThan(5);
        expect(attacks.length, `only saw: ${[...new Set(duel.reasons)].join(', ')}`).toBeGreaterThan(0);
    });

    it('uses more than one move rather than running the same attack forever', () => {
        const duel = runDuel(EVERY_MOVE);
        const moves = new Set(duel.reasons.filter(reason => reason !== 'impact' && reason !== 'out of bounds' && reason !== 'pinned'));

        expect(moves.size, `only used: ${[...moves].join(', ')}`).toBeGreaterThan(1);
    });

    it('never holds the player pinned underneath it', () => {
        const duel = runDuel(EVERY_MOVE);

        expect(duel.longestRideSeconds, `sat on the player for ${duel.longestRideSeconds.toFixed(2)}s`).toBeLessThan(2);
        ...
        expect(duel.ridingShare, `spent ${(duel.ridingShare * 100).toFixed(0)}% of the duel on top of the player`).toBeLessThan(0.2);
    });
