...
describe('TEST_SCENARIO_CATALOG', () => {
    it('keeps pinewood derby cars inside the track lanes with clearance', () => {
        ...
        const railPositions = scenario!.environment.obstacles
            ...
            .sort((a, b) => a - b);
        const laneWidth = Math.min(Math.abs(railPositions[1] - railPositions[0]), Math.abs(railPositions[2] - railPositions[1]));
        const carHalfWidth = Math.max(...PINEWOOD_DERBY_CAR_PRESET.state.parts.map(part => Math.abs(part.position.z) + part.dimensions.y / 2));

        expect(laneWidth).toBeGreaterThan(carHalfWidth * 2 + 0.3);
        expect(scenario!.participants.every(participant => participant.defaultAssemblyAssetId === 'pinewood-derby-car')).toBe(true);
    });
});
