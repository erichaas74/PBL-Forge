...
function egg(id: string, alleles: readonly [string, string]): DragonAnalysisSample {
  return {
    id,
    sampleType: 'egg',
    role: 'offspring',
    ...
    genes: [{
      ...
      chromosomeModel: 1,
      allelePair: [
        { id: `${id}:0`, geneId: 'W', symbol: alleles[0], parentSource: 'parent-a', expression: 'dominant' },
        { id: `${id}:1`, geneId: 'W', symbol: alleles[1], parentSource: 'parent-b', expression: 'recessive' },
      ],
...
describe('DragonHatcheryDisplayComponent', () => {
  ...
  it('stages an egg and commits the hatch through semantic events', () => {
    ...
    expect(host().querySelectorAll<HTMLButtonElement>('.stage-toggle')[0].disabled).toBeTrue();
    expect(host().querySelector<HTMLButtonElement>('[data-target="hatch-control"]')?.disabled).toBeTrue();
  ...
  it('lists hatched dragons without publishing an unsampled genotype', () => {
    render(scene('explain', {
      eggs: [
        { eggId: 'egg-1', sampleId: 'egg-1', position: 1, examined: false, sampled: false, hatched: false },
        { eggId: 'egg-2', sampleId: 'egg-2', position: 2, examined: false, sampled: false, hatched: true },
      ],
