...
function toEggView(input: EggViewInput): HatcheryEggView {
  ...
  const traits = phenotypeVisible && sample
  ...
  return {
    ...
    canExamine: input.toolsEnabled
      && input.availableTools.includes('examine')
      && !egg.examined && !egg.hatched && !locked
      && budgetLeft('examine'),
    canSample: input.toolsEnabled
      && input.availableTools.includes('sample')
      && !egg.sampled && !egg.hatched && !locked
      && budgetLeft('sample'),
    canStage: input.stagingEnabled
      && !egg.hatched && !locked
...
function focusTraitName(
  ...
  for (const sample of scene.samples) {
    const gene = sample.genes.find(candidate =>
      candidate.geneId === focusGeneId || candidate.traitId === focusGeneId);
