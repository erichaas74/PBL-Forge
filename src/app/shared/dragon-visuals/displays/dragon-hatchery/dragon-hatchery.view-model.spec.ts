...
function egg(id: string, wings: readonly [string, string], fire: readonly [string, string]): DragonAnalysisSample {
  return {
    id,
    sampleType: 'egg',
    role: 'offspring',
...
function gene(
  ...
  return {
    ...
    chromosomeModel,
    allelePair: [
      { id: `${sampleId}:${traitId}:0`, geneId, symbol: alleles[0], parentSource: 'parent-a' as const, expression: 'dominant' as const },
      { id: `${sampleId}:${traitId}:1`, geneId, symbol: alleles[1], parentSource: 'parent-b' as const, expression: 'recessive' as const },
    ] as const,
    phenotypeId,
  };
}
...
function sceneWith(
  ...
  return {
    ...
    instrument: {
      ...
      focusGeneId: 'W',
      eggs: [
        { eggId: 'egg-1', sampleId: 'egg-1', position: 1, examined: false, sampled: false, hatched: false },
        { eggId: 'egg-2', sampleId: 'egg-2', position: 2, examined: false, sampled: false, hatched: false },
        { eggId: 'egg-3', sampleId: 'egg-3', position: 3, examined: false, sampled: false, hatched: false },
      ],
      activeEggId: 'egg-1',
      selectedEggIds: [],
      hatchLimit: 2,
...
describe('Dragon hatchery view model', () => {
  ...
  it('publishes the trait readout after examining, and the allele pair only after sampling', () => {
    const examined = buildDragonHatcheryViewModel(
      sceneWith('manipulate', {
        eggs: [
          { eggId: 'egg-1', sampleId: 'egg-1', position: 1, examined: true, sampled: false, hatched: false },
        ],
      }),
      COPY,
    );
    ...
    expect(examined?.eggs[0].traits.map(trait => trait.phenotype))
      .toEqual(['Winged', 'Breathes fire']);
    ...
    const sampled = buildDragonHatcheryViewModel(
      sceneWith('manipulate', {
        eggs: [
          { eggId: 'egg-1', sampleId: 'egg-1', position: 1, examined: true, sampled: true, hatched: false },
        ],
      }),
      COPY,
    );
  ...
  it('reveals what a hatched dragon shows without revealing its alleles', () => {
    const model = buildDragonHatcheryViewModel(
      sceneWith('reveal', {
        eggs: [
          { eggId: 'egg-1', sampleId: 'egg-1', position: 1, examined: false, sampled: false, hatched: true },
        ],
  ...
  it('locks an egg the module withheld', () => {
    const model = buildDragonHatcheryViewModel(
      sceneWith('manipulate', {
        eggs: [
          { eggId: 'egg-1', sampleId: 'egg-1', position: 1, examined: false, sampled: false, hatched: false, locked: true },
        ],
      }),
      COPY,
    );
  ...
  it('counts the clutch and describes it for a screen reader', () => {
    const model = buildDragonHatcheryViewModel(
      sceneWith('manipulate', {
        eggs: [
          { eggId: 'egg-1', sampleId: 'egg-1', position: 1, examined: true, sampled: true, hatched: false },
          { eggId: 'egg-2', sampleId: 'egg-2', position: 2, examined: true, sampled: false, hatched: false },
          { eggId: 'egg-3', sampleId: 'egg-3', position: 3, examined: false, sampled: false, hatched: false },
        ],
