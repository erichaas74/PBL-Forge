 ...
 * accessible text. Set `concealed` to cover the focus locus with a sample shield, and use
    ...
    >
      ...
      @if (concealed()) {
        <g class="shield">
          ...
          <g
            [attr.transform]="
              'translate(' + (theme().viewportWidth / 2 - 6) + ',' + (bandCentre() - 7) + ')'
            "
          >
...
export class ChromosomePairComponent {
  ...
  readonly viewBox = computed(() => chromosomeViewBox(this.theme(), this.geneIndex(), this.view()));
