...
function part(id: string, roles: AssemblyPartRole[], position: { x: number; y: number; z: number }): AssemblyPart {
  return {
    id,
    roles,
    shape: 'box',
...
function positionOf(pose: { parts: readonly { partId: string; position: { x: number; y: number; z: number } }[] }, id: string) {
    ...
    for (const ability of ['bite', 'wing-buffet', 'tail-sweep', 'fire-breath'] as const) {
      for (const phase of [0, 1]) {
        ...
        for (const entry of rest.parts) {
          ...
          expect(distance(moved, entry.position))
            .withContext(`${ability} at phase ${phase} left ${entry.partId} displaced`)
            .toBeLessThan(0.02);
        }
      }
    }
  ...
  it('keeps chains joined at their pivots throughout a demo', () => {
    ...
    for (const phase of [0.2, 0.5, 0.8]) {
      ...
      for (const link of blueprint.joints) {
        ...
        expect(distance(worldPoint(parent, link.pivotOnParent), worldPoint(child, link.pivotOnChild)))
          .withContext(`${link.id} came apart at phase ${phase}`)
          .toBeLessThan(1e-6);
      }
    }
  });
