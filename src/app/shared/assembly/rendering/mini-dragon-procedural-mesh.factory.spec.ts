...
import { MINI_DRAGON_PROFILE_IDS, createMiniDragonProceduralObject, isMiniDragonProfileId, miniBodySurfaceNormal, miniBodySurfacePoint, sampleMiniBodyRadius, } from './mini-dragon-procedural-mesh.factory';

function part(profileId: string, overrides: Partial<AssemblyPart> = {}, parameters: Record<string, string | number | boolean> = {}): AssemblyPart {
    return {
        id: `test-${profileId}`,
        shape: 'box',
        mass: 1,
...
describe('mini dragon procedural mesh factory', () => {
    ...
    describe('body', () => {
        ...
        it('renders smooth rows or rounded baby spikes from the inherited back-scale form', () => {
            const smooth = createMiniDragonProceduralObject(part('mini-dragon-dorsal-scales', {}, { miniDorsalBumps: 0 }))!;
            const bumpy = createMiniDragonProceduralObject(part('mini-dragon-dorsal-scales', {}, { miniDorsalBumps: 1 }))!;
        ...
        it('adds coat patches only when the specimen is two-toned', () => {
            const plain = createMiniDragonProceduralObject(part('mini-dragon-body', { color: '#c8a24a' }, { miniPatchColor: '#c8a24a' }));
            const patched = createMiniDragonProceduralObject(part('mini-dragon-body', { color: '#c8a24a' }, { miniPatchColor: '#3b2a1c' }));
        ...
        it('draws inherited body feathers as one bounded instanced alpha-card layer', () => {
            const bare = createMiniDragonProceduralObject(part('mini-dragon-body', {}, { miniFeatherCoverage: 0 }))!;
            const feathered = createMiniDragonProceduralObject(part('mini-dragon-body', {}, { miniFeatherCoverage: 1 }))!;
        ...
        it('uses feather coverage to change instance density without adding draw layers', () => {
            const fringe = createMiniDragonProceduralObject(part('mini-dragon-body', {}, { miniFeatherCoverage: 0.55 }))!;
            const full = createMiniDragonProceduralObject(part('mini-dragon-body', {}, { miniFeatherCoverage: 1 }))!;
    ...
    describe('head', () => {
        ...
        it('renders crown bumps and side frills as separate codominant crest geometry', () => {
            const head = createMiniDragonProceduralObject(part('mini-dragon-head', {}, { miniCrestCrown: 1, miniCrestFrill: 1 }))!;
        ...
        it('keeps the eyes on the cranium rather than inside it', () => {
            ...
            const offset = (eye.position.x / (skullRadius * (dimensions.x / dimensions.y) * 0.92)) ** 2 +
        ...
        it('sweeps a curled horn further than a straight one', () => {
            const straight = createMiniDragonProceduralObject(part('mini-dragon-head', {}, { miniHornCurl: 0 }))!;
            const curled = createMiniDragonProceduralObject(part('mini-dragon-head', {}, { miniHornCurl: 1 }))!;
    ...
    describe('wing', () => {
        it('builds a membrane and struts at full spread', () => {
            const wing = createMiniDragonProceduralObject(part('mini-dragon-wing', {}, { miniWingSpread: 1, miniWingSide: 1 }))!;
        ...
        it('collapses to a furred nub for the vestigial genotype', () => {
            const wing = createMiniDragonProceduralObject(part('mini-dragon-wing', {}, { miniWingSpread: 0.12, miniWingSide: -1 }))!;
        ...
        it('mirrors on the side the part declares, not on its name', () => {
            const left = createMiniDragonProceduralObject(part('mini-dragon-wing', { id: 'anything' }, { miniWingSpread: 1, miniWingSide: -1 }))!;
            const right = createMiniDragonProceduralObject(part('mini-dragon-wing', { id: 'anything' }, { miniWingSpread: 1, miniWingSide: 1 }))!;
        ...
        it('adds one instanced feather layer to a feathered functional wing', () => {
            const wing = createMiniDragonProceduralObject(part('mini-dragon-wing', {}, { miniWingSpread: 1, miniWingSide: 1, miniFeatherCoverage: 1 }))!;
    ...
    describe('leg and tail', () => {
        ...
        it('builds a shank, a paw, and soft toes with no talons', () => {
            const leg = createMiniDragonProceduralObject(part('mini-dragon-leg', {}, { miniToeCount: 4 }))!;
        ...
        it('grows a fuller rounded pom when its profile requests one', () => {
            const shallow = createMiniDragonProceduralObject(part('mini-dragon-tail-plume', {}, { miniTailStyle: 2, miniPlumeFan: 0 }))!;
            const full = createMiniDragonProceduralObject(part('mini-dragon-tail-plume', {}, { miniTailStyle: 2, miniPlumeFan: 1 }))!;
        ...
        it('builds three unmistakably different inherited tail tips', () => {
            const star = createMiniDragonProceduralObject(part('mini-dragon-tail-plume', {}, { miniTailStyle: 0 }))!;
            const fork = createMiniDragonProceduralObject(part('mini-dragon-tail-plume', {}, { miniTailStyle: 1 }))!;
            const pom = createMiniDragonProceduralObject(part('mini-dragon-tail-plume', {}, { miniTailStyle: 2 }))!;
