...
function membraneOf(object: THREE.Object3D): THREE.Mesh {
    ...
    if (!densest)
        throw new Error('no mesh in wing');
    return densest;
}
...
function skinOf(object: THREE.Object3D): THREE.MeshStandardMaterial {
    ...
    if (!found)
        throw new Error('no standard material');
    return found;
}
...
function headPart(profileId: 'dragon-head-horned', shape: 'sphere' | 'box', dimensions: AssemblyPart['dimensions']): AssemblyPart {
    return {
        id: profileId,
        label: profileId,
        roles: ['head'],
...
function skullOf(object: THREE.Object3D): THREE.Mesh {
    ...
    object.traverse(child => {
        if (!(child instanceof THREE.Mesh))
            return;
        const count = child.geometry.getAttribute('position')?.count ?? 0;
        if (count > (densest?.geometry.getAttribute('position')?.count ?? -1))
            densest = child;
    });
    if (!densest)
        throw new Error('no mesh in head');
    return densest;
}
...
describe('dragon head mesh', () => {
    ...
    it('lofts a skull that responds to all three dimensions', () => {
        const round = skullOf(createDragonProceduralObject(headPart('dragon-head-horned', 'box', { x: 0.5, y: 0.4, z: 0.4 }))!);
        const long = skullOf(createDragonProceduralObject(headPart('dragon-head-horned', 'box', { x: 1.1, y: 0.4, z: 0.4 }))!);
    ...
    it('sizes a sphere head from its radius, not its diameter', () => {
        const head = createDragonProceduralObject(headPart('dragon-head-horned', 'sphere', { x: 0.42, y: 0.42, z: 0.42 }))!;
    ...
    it('keeps the skull inside the physics volume', () => {
        ...
        const bounds = new THREE.Box3().setFromObject(skullOf(createDragonProceduralObject(headPart('dragon-head-horned', 'box', dims))!));
    ...
    it('gives the skull UVs so the scale texture lands on it', () => {
        const skull = skullOf(createDragonProceduralObject(headPart('dragon-head-horned', 'box', { x: 0.68, y: 0.38, z: 0.34 }))!);
    ...
    it('grows nothing where a hornless skull would carry horns', () => {
        ...
        const meshes = (head: THREE.Object3D) => {
            let count = 0;
            head.traverse(child => { if (child instanceof THREE.Mesh)
                count += 1; });
            return count;
        };
...
describe('dragon body mesh', () => {
    ...
    it('gives alternate body plans distinct anatomical silhouettes', () => {
        ...
        expect(bodyFor('four-wing').children.filter(child => child.name.startsWith('dragon-body-four-wing-scapula')).length)
            .toBe(4);
        expect(bodyFor('classic').children.some(child => child.name.includes('wyvern-keel'))).toBe(false);
    });
});
...
describe('dragon male frill', () => {
    ...
    function spines(head: THREE.Object3D): THREE.Object3D[] {
        ...
        head.traverse(child => {
            if (child.name.startsWith('dragon-male-crest-spine-'))
                found.push(child);
        });
        return found;
    }
    ...
    it('stands every spine tip out past the membrane it carries', () => {
        ...
        for (const spine of spines(head))
            ring.union(new THREE.Box3().setFromObject(spine));
...
describe('dragon grasping forelimb', () => {
    ...
    it('builds an arm segment with a joint ball at each end', () => {
        const arm = createDragonProceduralObject(limbPart('dragon-leg', { x: 0.14, y: 0.33, z: 0.14 }));
    ...
    it('is slimmer than the walking leg it replaces', () => {
        ...
        const leg = new THREE.Box3().setFromObject(childNamed(createDragonProceduralObject(limbPart('dragon-leg', dims))!, 'dragon-leg-skin'));
        const arm = new THREE.Box3().setFromObject(childNamed(createDragonProceduralObject({
    ...
    it('tips each finger with a claw of its own', () => {
        ...
        const knuckle = new THREE.Box3().setFromObject(childNamed(hand, 'dragon-grasp-finger-2-knuckle'));
    ...
    it('grows the fingers with the claw gene, the same one the feet read', () => {
        ...
        const clawed = createDragonProceduralObject(handPart({
            visualProfile: {
        ...
        const reach = (object: THREE.Object3D): number => new THREE.Box3().setFromObject(childNamed(object, 'dragon-grasp-finger-2')).max.x;
...
describe('dragon joint balls', () => {
    ...
    it('scales the balls with the part, not in world units', () => {
        const small = createDragonProceduralObject(limbPart('dragon-leg', { x: 0.22, y: 0.72, z: 0.22 }))!;
        const large = createDragonProceduralObject(limbPart('dragon-leg', { x: 0.44, y: 1.44, z: 0.44 }))!;

        const smallBall = ballBounds(small, 'dragon-leg-socket-ball');
        const largeBall = ballBounds(large, 'dragon-leg-socket-ball');

        expect(largeBall.max.x - largeBall.min.x).toBeCloseTo((smallBall.max.x - smallBall.min.x) * 2, 4);
    });

    it('takes a per-part override for the ball width', () => {
        const dims = { x: 0.22, y: 0.72, z: 0.22 };
        const standard = ballBounds(createDragonProceduralObject(limbPart('dragon-leg', dims))!, 'dragon-leg-socket-ball');
        const wide = ballBounds(createDragonProceduralObject({
            ...limbPart('dragon-leg', dims),
            visualProfile: {
    ...
    it('closes the throat where the skull hinges on the torso', () => {
        const head = createDragonProceduralObject(headPart('dragon-head-horned', 'sphere', { x: 0.42, y: 0.32, z: 0.3 }))!;
...
describe('dragon wing membrane', () => {
    ...
    it('carries usable normals, so lighting reads the curvature', () => {
        ...
        for (let index = 1; index < normals.count; index += 1) {
            ...
            if (first.distanceTo(other) > 0.05) {
                varies = true;
                break;
            }
        }
