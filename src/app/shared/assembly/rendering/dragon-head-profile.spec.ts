...
import { DEFAULT_HEAD_SHAPE, HEAD_SHAPES, HEAD_SHAPE_BY_PROFILE, dragonHeadExtent, dragonHeadEyeSocket, dragonHeadJawMount, dragonHeadHornMount, dragonHeadJawMountFor, dragonHeadNostril, dragonHeadSection, dragonHeadStations, dragonHeadSurfacePoint, headShapeFor, headShapeForProfile, } from './dragon-head-profile';
...
function envelope(shape = DEFAULT_HEAD_SHAPE): {
    height: number;
    width: number;
} {
    let height = 0;
...
describe('head variant table', () => {
    ...
    it('gives every variant in the table a skull of its own', () => {
        ...
        const depths = profileIds.map(profileId => dragonHeadSection(dims, 0.42, headShapeForProfile(profileId, dims)).halfHeight);
