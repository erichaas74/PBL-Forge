...
import { getAssemblyRenderSignature } from './three-assembly-mesh.factory';
...
describe('getAssemblyRenderSignature', () => {
  it('changes for procedural parameters and profile rotation', () => {
    ...
    const parameterChange = getAssemblyRenderSignature(part({
      visualProfile: {
        profileId: 'dragon-body',
        meshType: 'procedural',
    ...
    const rotationChange = getAssemblyRenderSignature(part({
      visualProfile: {
        profileId: 'dragon-body',
        meshType: 'procedural',
        parameters: { spikeCount: 3, camber: 0.2 },
  ...
  it('changes when a geometry-affecting role changes', () => {
    expect(getAssemblyRenderSignature(part({ roles: ['wheel'] })))
      .not.toBe(getAssemblyRenderSignature(part({ roles: [] })));
  });
