...
function uRange(geometry: THREE.BufferGeometry, filter?: (index: number) => boolean): [number, number] {
...
describe('dragon texture cache', () => {
  ...
  it('produces a normal map with real relief, not a flat sheet', () => {
    ...
    for (let index = 0; index < pixels.length; index += 4) {
      if (Math.abs(pixels[index] - 128) > 12 || Math.abs(pixels[index + 1] - 128) > 12) tilted += 1;
    }
