...
function buildWing(part: AssemblyPart): THREE.Group {
  return buildDragonWing(part, dragonPaletteForPart(part));
}
...
describe('dragon wing membrane', () => {
    ...
    it('is a curved surface, not a flat sheet', () => {
        const object = buildWing(wingPart());
    ...
    it('has interior vertices to curve, not just an outline', () => {
        const membrane = membraneOf(buildWing(wingPart())!);
    ...
    it('scales its relief with the genome, not a fixed size', () => {
        const small = membraneOf(buildWing(wingPart())!);
        const large = membraneOf(buildWing(wingPart({
    ...
    it('mirrors the right wing rather than building a second mesh', () => {
        const right = buildWing(wingPart({
            id: 'right-wing',
            label: 'Right Wing',
            snapPoints: [{
                    id: 'dragon-wing-root',
